...
- **Angle/Brief = PERSONA × ROUTE.** `ssc-ads-brief` judges which personas
  ...
  > **The mechanism guarantee is one ANGLE, one mechanism** — never *one
  > subject, one mechanism*. It is settled once on the subject and inherited by
  > every angle beneath it, but an angle brief may author a **bounded
  > angle-local override** where the inherited mechanism does not serve that
  > angle's persona × route: bank-first from `craft/mechanism-bank` read live,
  > judged against `craft/doctrine` §2, grounded in an attributed
  > voice-of-customer item from the approved Approaches doc, proof-routed from
  > the period's stated inventory and dropped if compliance refuses that route,
  > always reported naming its `bank_id` or `in_bank: false`. Its blast radius is
  > ONE angle: `idea.mechanism` is never written, patched or demoted and no
  > sibling angle is re-opened. Mechanisms are drawn **bank-first** from the
  > standing `craft/mechanism-bank`, not authored fresh each period — invention
  > is gap-fill only, and is marked `in_bank: false` so it stays visible.
...
| Knowledge base | `/ssc-kb` | `ssc-kb-agent` | review → audit → research → revise / gap-fill, plus **harvest** (`ssc-kb-mechanism-harvest`) — a standalone branch that diffs a period's settled mechanisms against the live `craft/mechanism-bank` and folds the run into ONE propose-only revision of that document; it writes no usage history and retires nothing. Research **persists nothing**: there is no `research` table and no `save_research` tool, so the report's own source lines are the provenance a revision's `evidence_note` carries |
