...
**Reason**: The bank is no longer a knowledge-base document. The BrandOS server promoted
it into a `mechanisms` table with its own three tools (`list_mechanisms`,
`get_mechanism`, `save_mechanism`) plus the generic `edit` / `approve` / `delete`, and
seeded it with approved entries. A three-section Markdown contract whose `id` uniqueness
and valence legality were "enforced by nothing" no longer describes where entries live or
how they are governed.

**Migration**: Replaced by two ADDED requirements — "The bank is a `mechanisms` table read
and written through its own tools", which carries the tool surface, the approved-only read
default and the draft-minting rule; and "The `craft/mechanism-bank` document describes the
bank it no longer stores", which keeps §1's pointer to `craft/doctrine` §2 and §2's
valence vocabulary in the document while §3 becomes a description of the table's fields
and §4 becomes what to do when no entry fits. Skills that read `craft/mechanism-bank` §3
for entries call `list_mechanisms` instead.

### Requirement: The candidate supply is built bank-first, and every drawn candidate names its `bank_id`

**Reason**: There is no candidate supply. `ssc-approaches-core` no longer reads the bank
and no longer proposes mechanisms; the step that actually chooses one — the angle brief —
now queries the table directly, so a per-period shortlist would be a second opinion the
brief would have to reconcile with the bank it is reading anyway.

**Migration**: Bank-first ordering moves to the new `angle-mechanism-authoring`
requirement "The mechanism is settled bank-first from the `mechanisms` table". `bank_id`
is gone with the supply: provenance is now the entry's `slug`, carried in the run's
report only — there is no `briefs.mechanism_slug` column.

### Requirement: Gap-fill is the only invention the core makes, and it is visibly marked

**Reason**: The core makes no mechanism at all, so it fills no gap. The `in_bank: false`
marker existed to distinguish a supplied draw from a supplied invention, and both halves
of that distinction left Approaches with the supply.

**Migration**: Authoring-only-where-nothing-fits moves to `angle-mechanism-authoring`'s
bank-first requirement; the visibility obligation moves to its "Every run reports the
mechanism and its provenance" requirement, which reports either the drawn `slug` or that
the mechanism is not in the bank. No skill carries an `in_bank` field.

### Requirement: Every other supply constraint is unchanged by the bank

**Reason**: The requirement's subject — the candidate supply and its volume floors (one
candidate per featured persona; enough candidates that none would carry more than about a
quarter of the period's assets) — no longer exists. Sizing a supply is meaningless when no
supply is produced.

**Migration**: The constraints that were about a *mechanism* rather than about the supply
survive in `angle-mechanism-authoring`: proof-routing from the period's stated inventory,
`unverified_for_period` where the inventory is absent, and the drop-not-soften-not-retrace
rule on a compliance refusal, applied to a bank draw exactly as to an authored mechanism.
The quarter-of-planned-assets figure survives as the concentration threshold in harvest's
report-only period mix audit. The per-persona floor is subsumed by `creative_target`,
which `ssc-ads-approaches` already owns.

### Requirement: The return shape gains exactly two fields and loses none

**Reason**: The `candidate_mechanisms` block it describes is deleted along with the
supply, so there is no return shape for `bank_id` and `valence` to widen.

**Migration**: None needed. The two facts the fields carried are now read directly by the
consumer that needs them — `valence` from the bank entry itself via `list_mechanisms`,
and provenance from the run's report.

### Requirement: Ideate carries `bank_id` through onto the mechanism it settles

**Reason**: Ideate settles no mechanism. `ideas.mechanism` was dropped and `save_idea`
accepts no `mechanism`, so there is nothing for Ideate to carry provenance onto.

**Migration**: Covered by `angle-mechanism-authoring`'s "Ideate touches no mechanism at
all". The drafting-is-never-blocked and grandfathering rules this requirement restated
move to that capability's approval-gate requirement, retargeted from the idea to the
brief. `ssc-post-ideate` round 2 additionally withholds `detail.mechanism` at mint so the
brief mints `draft` rather than `approved`.

### Requirement: Negative-valence mechanisms carry no more than a third of the period's assets

**Reason**: The cap was enforced at Ideate, which no longer writes or owns a mechanism —
it would be counting a field it does not write on rows it does not own. Re-mechanising
"from the supply's positive candidates" is also unreachable now that there is no supply.

**Migration**: Replaced by the ADDED requirement "The period's mechanism mix is audited at
harvest and reported, never enforced", which keeps both ratios — the `~¼` per-mechanism
concentration threshold and the `⅓` negative-valence threshold — measures them over the
period's **briefs**, names every breach, and leaves the correction to the operator on
not-yet-approved briefs. Nothing re-mechanises automatically, and no mechanism is invented
to satisfy a count.

### Requirement: A near-duplicate is proposed as a revision of the existing entry

**Reason**: `propose_knowledge_revision` no longer reaches bank entries — they are table
rows, not document sections — and the operator chose bounded in-place sharpening over a
report-only path.

**Migration**: Replaced by the ADDED requirement "A near-duplicate is sharpened in place,
bounded to content fields and always reported", which uses `edit(entity='mechanism')`
under three bounds: content fields only (`mechanism`, `fits`, `proof_family`, `notes`),
never `status` and never `slug`; sharpening never repurposing; and every edit reported
with its before and after. Naming the matched entry and the reason is preserved verbatim
from the removed requirement.

### Requirement: The harvest skill is propose-only and retires nothing

**Reason**: The declared tool list is wrong in both directions — the skill no longer holds
`propose_knowledge_revision` (nothing about the bank goes through a knowledge revision
any more) and it does now hold `save_mechanism` and a bounded `edit`. Calling it
"propose-only" would overstate the governance position and hide the one deliberate
loosening this change makes.

**Migration**: Replaced by the ADDED requirement "The harvest skill holds no approval
verb, mints only drafts, and retires nothing", which keeps the invariants that still hold
— no `approve`, no `unapprove`, no publish or schedule tool, no usage history, retires
nothing, weak entries reported as findings — and states the two writes it does have,
each of which stops short of the governance line: `save_mechanism` can only mint a draft,
and `edit` is bounded to content fields.
