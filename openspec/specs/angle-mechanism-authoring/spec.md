# angle-mechanism-authoring

## Purpose

Where a mechanism is settled and by what rules. The mechanism is authored at the
**angle brief** and lives in `briefs.mechanism` alone — there is no mechanism on the
idea, no inheritance, no override and no resolution step. The guarantee is **one angle,
one mechanism**, and sibling angles of one subject may disagree. Every mechanism is
settled bank-first from the `mechanisms` table, judged against `craft/doctrine` §2 read
live, grounded in an attributed voice-of-customer item from the approved Approaches
document, proof-routed from the period's inventory, and dropped rather than softened
when compliance refuses its only route. `ssc-brief-core` reasons and holds no mutation
tool; the caller saves. Ideate touches no mechanism at all, producers read
`brief.mechanism` alone, and the approval bar lives server-side on
`approve(entity='brief')` for `ad` and `post`.
