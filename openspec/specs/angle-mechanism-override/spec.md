# angle-mechanism-override

## Purpose

The bounded permission for an angle brief to carry its own mechanism, for the case
where the one settled on its idea does not serve that angle's persona x route. The
guarantee is **one angle, one mechanism** — never one subject, one mechanism. The
mechanism is settled once on the persona-free subject and inherited by every angle
beneath it; an override is the exception, bank-first, doctrine-judged, VOC-grounded,
proof-routed, and dropped rather than softened when compliance refuses its only route.
Its blast radius is ONE angle: `idea.mechanism` is never written, patched or demoted,
and no sibling angle is re-opened. Every production consumer resolves the brief's
override first, then the idea's.
...
> **STATUS — NOT YET DEPLOYED (as of 2026-08-03).** This requirement describes a
> contract the plugin is built against, **not** a capability any running server
> provides. `briefs.mechanism` exists only as uncommitted work in the `content`
> repository; no deployed BrandOS server carries it. Until it ships, the requirement
> immediately below — *"an override is reported and not persisted"* — is the behaviour
> actually in force, and it is the one to read when reasoning about live runs. Do not
> build on this contract without first confirming the deployed tool schemas carry the
> field (check the pod's `startTime` against the commit; a session caches tool schemas
> at start).
>
> **One clause here is already retracted.** `get_idea` and `list_ideas` **do** return
> `mechanism`, and have since `c391926` (2026-08-01) — before the change that wrote
> this spec opened. The "which do not return it today" claim below was verified wrong;
> see the archived change's `design.md` Drift Log DL2. That half of the contract is
...
(**already satisfied** — see the status note above).
...
title.~~ (Retracted with the clause it justified: the idea's `mechanism` is readable, and
`ssc-post-ideate` reads it back.) Making `mechanism` approval-bearing, by contrast, would
invent a second gate on a field that carries no spend and no publish consequence, and
would drag the governance hook into a change that has no governance content.
...
> **STATUS — THIS IS THE BEHAVIOUR IN FORCE (as of 2026-08-03).** `briefs.mechanism` is
> not deployed, so every override authored on a live run today is reported and not
> persisted. Read this requirement, not the contract above, when reasoning about what
> actually happens.
