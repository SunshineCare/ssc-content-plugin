# approaches-shared-core

## Purpose

`ssc-approaches-core`, the shared channel-agnostic core both Approaches steps dispatch:
it carries the quarter's market-sophistication read through verbatim and compiles the
per-persona voice-of-customer pass, every quoted line attributed to a recorded source and
every silent source named as a gap. It is view-only — it holds no mutation tool, reads no
plan state, runs no outward pass, and returns text rather than a document, so each channel
keeps its own artifact shape and its own gates. `channel` is its only conditional: a
`post` run binds every quoted line to `rules/organic-vs-paid-firewall` and refuses
ad-sourced material. The approved document its caller composes is the sanctioned source of
the attributed voice-of-customer item a mechanism is later grounded in at the angle brief.
...
SHALL NOT approve, unapprove or flip any gate.
...
additionally bind every quoted line to `rules/organic-vs-paid-firewall`, read live,
and SHALL refuse to source any voice-of-customer quote or example from ad copy or
from the ad performance lens. For `channel='ad'` it SHALL behave as the Ads
Approaches step behaves today. No other input SHALL introduce a branch.
...
#### Scenario: An ad run binds no firewall

- **WHEN** the core runs with `channel='ad'`
- **THEN** it applies no firewall binding
- **AND** it returns the same blocks it returns for any other channel
...
and SHALL NOT carry their own copy of the sophistication-inherit rule or the
voice-of-customer pass. The Ads Approaches persisted document SHALL keep its section
...
- **THEN** it dispatches the core for both pieces of work
- **AND** it does not restate the sophistication-inherit rule or the
  voice-of-customer pass

#### Scenario: The Ads artifact keeps its shape

- **WHEN** the Ads Approaches step persists its document
- **THEN** the document follows the section shape and wording rules that step owns
...
The core SHALL name every knowledge-base document it applies — `craft/doctrine` §6's
rule-ownership table (and **not** §2's mechanism definition, which belongs to the step
that settles a mechanism), the persona index and each currently-listed persona detail
doc, the banned-words document, and for `channel='post'` the organic-versus-paid
firewall — and SHALL read each live at run time. It SHALL restate none of their
contents: no persona name, trigger, prohibition or ladder value SHALL appear in the
skill file. A failed read SHALL stop the run, SHALL name the document that could not
be read, and SHALL NOT fall back to a remembered version.
