# mechanism-proof-substantiation

## Purpose

What binds proof to mechanism in the writing skills. A `copy` variation's mechanism
beat — written from `brief.mechanism` — leans on at least one row of the live
`brand/proof-points`, and the record names which row. The search starts in the proof
family the mechanism's own claim argues from, read from the mechanism sentence against
the live doc rather than looked up in the `mechanisms` bank, and may reach beyond that
family outright — marked when it does, so a corroboration is never mistaken for the
re-trace `angle-mechanism-authoring` forbids. Proof enhances the hook rather than merely
answering it, and a proof line surviving the competitor-swap test is cut at composition
and replaced by the concrete form the live row carries — cutting the line, never the
variation. The force is a composition rule plus a brand-fit cap at ≤3: no floor item, no
