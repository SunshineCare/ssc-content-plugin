...
After the writer saves a section's drafts, point the operator to `/ad/[month]/[id]` for the concept to **review / edit / approve** that section — then **re-run this command** for the same `idea_id` to produce the next section (headline → copy → description → image_content). When all four sections have an approved variation, production is complete. Re-invoke per concept — it works ONE approved concept at a time.
