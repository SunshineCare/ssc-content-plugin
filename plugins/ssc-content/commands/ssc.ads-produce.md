...
`ssc-ads-brief` revises the idea's own creative-brief fields (`hook_direction`/`core_message`/`why_now`/`story_moment`/`cta` — never `theme`, which ad ideas don't carry) — no `content` row, no draft/approve step. It requires `copy` to be approved and directly overwrites the five fields each run, via the generic `update_idea` tool (id + expected_version + the five fields), with values grounded in whatever is currently approved, so re-running it after more sections get approved refreshes the brief.
