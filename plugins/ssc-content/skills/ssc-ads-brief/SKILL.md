...
**Next:** sharpen the concept (`title` / `tags` / `ad_notes`) if you genuinely want more angles, then re-invoke `/ssc.ads-brief <idea_id>`. Otherwise, approve the angle(s) worth producing in /ad/<month>/<idea_id> and run `/ssc.ads-produce <idea_id> <brief_id>`.
```
