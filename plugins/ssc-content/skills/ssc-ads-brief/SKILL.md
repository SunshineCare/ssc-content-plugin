...
> **Server dependency — the multi-angle payoff is inert until "Change 2" (read this — avoid assuming a capability that has not shipped).** On today's BrandOS server, `save_brief.angle_label` is nulled and `list_briefs` returns **exactly one row per idea** ("N per angle once **Change 2** lands"). So today the server collapses your 4-5 `save_brief` calls to a **single persisted brief with a null `angle_label`**, and the produce-once guard trips on the next run. This skill is written for the **target (Change-2) model** — it produces up to five angles and sets `angle_label` on each, which is **forward-compatible**: passing `angle_label` is harmless today (the server nulls it) and correct the moment Change 2 lands, so no second edit is needed. The skill still runs correctly today; only the multi-angle payoff is inert — the same pattern the video skills use for not-yet-shipped server capabilities. Stated plainly here, never assumed silently.

This is the **first step** of the ad flow — it runs right after the concept is approved (`ssc-ads-agent`'s Ideate), to give the operator a set of synthesized angle options to pick from before production. It does **not** gate `ssc-image` — that skill's precondition remains an approved `image_content` variant only.
...
It returns the idea's briefs (today's server: exactly one row per idea; N per angle once Change 2 lands). If it returns **≥1 brief**, STOP and tell the operator:

> This concept already has creative-brief angle(s) — curate / approve / discard them in `/ad/[month]/[id]`. This skill produces the angle set **once** and won't append or overwrite. To regenerate from scratch, discard all briefs in the dashboard first, then re-invoke `/ssc.ads-brief <idea_id>`.
...
**The `angle_label` (MANDATORY, distinct per angle).** A short Vietnamese label naming *this* angle's trigger point / objection / myth — the one-glance name a curator reads to tell the angles apart (e.g. `Nỗi sợ chùng da khi giảm cân`, `Phản bác: "phải nhịn ăn mới giảm được"`, `Gỡ lầm tưởng thực phẩm thay thế = thiếu chất`). Every angle carries one; **no two labels in the set are the same.** It is **always passed to `save_brief`** (forward-compatible — the server nulls it until Change 2 ships).
...
`save_brief` INSERTS a brief **always created as `draft`** — it cannot mint an approved brief and takes **no `status` argument**. One insert per ≥4 angle. Do NOT pass `theme` (removed from the schema entirely) and do NOT pass any approval/status field. Capture each returned confirmation for the Step 7 summary. Then **STOP** — you are done for this invocation. The operator reviews and **approves one** (or curates) in the `/ad/[month]/[id]` dashboard, then produces copy from it via `/ssc.ads-produce <idea_id> <brief_id>`.

> **Server note (Change 2):** on today's server the 4-5 `save_brief` calls collapse to a **single persisted brief** with a **null `angle_label`** (the server keeps one brief per idea and nulls the label until Change 2 ships). Passing `angle_label` and emitting the full set anyway is forward-compatible — correct the moment Change 2 lands, harmless before. Note in the summary how many angles you produced versus what today's server will persist.
...
**Server (Change 2):** produced <N> angles; today's server persists 1 brief with a null angle_label until Change 2 ships (forward-compatible).
**Next:** open /ad/<month>/<idea_id> → review the angle(s), **approve ONE** to produce from (or curate/discard), then run `/ssc.ads-produce <idea_id> <brief_id>` to write copy anchored to it. To regenerate the set, discard all briefs there first, then re-invoke `/ssc.ads-brief <idea_id>` — this skill produces once and refuses to append.
```
...
- **Produce-once guard (hard rule).** `list_briefs(idea)` runs **before any `save_brief`** (Step 2); if it returns ≥1 brief, STOP and route the operator to curate / approve / discard in the dashboard. This is the ONLY thing preventing brief pile-up once Change 2 allows N-per-idea — it MUST precede every write. Regeneration = discard all in the dashboard, then re-invoke; the skill never appends or overwrites.
...
- **Server Change-2 dependency (documented, not hidden).** Today's BrandOS server persists exactly one brief per idea and nulls `angle_label`; the multi-angle payoff is **inert until Change 2 ships**. The skill still runs correctly today (one brief persists), sets `angle_label` forward-compatibly, and states this dependency in its body — the same pattern the video skills use for not-yet-shipped server capabilities. No `plugin.json` / `.mcp.json` change (all tools land on the existing `ssc` surface).
- **Does not gate `ssc-image`.** That skill's precondition remains an approved `image_content` variant only; the creative-brief angles are a human-facing handoff artifact, not a machine gate.
