...
- **Never approves.** Does not set `tactics_approved`, `approved`, or any approval flag. Flipping the Focus gate is a dashboard-only action requiring the `approve` capability (via `approve_channel_plan`, gate `tactics`).
