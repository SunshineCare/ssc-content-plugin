...
**Save-to-server, not present-in-chat (the core of this flow):** after the quality loop leaves the active section's variations all rated ≥4, you **immediately SAVE each as a DRAFT `content` row** via `save_post_content` (`channel='ad'`, `idea_id`, `brief_id` — the chosen angle brief this run is anchored to — the active `section`, `body`, `score`, `comment`) and **STOP**. **`brief_id` is passed on EVERY save, in all four sections** (`copy`, `headline`, `description`, `image_content`), so each saved ad row records the angle it was written from (see Step 8 — omit it and the server **infers** an angle for you, out of the idea's several approved briefs, and can stamp the wrong one silently). You do **NOT** present a candidate set in chat, pause, or run an in-chat revise loop. The operator **reviews / edits / approves** the saved drafts in the `/ad/[month]/[id]` dashboard, then **re-invokes** you for the next section.
...
- Plan lineage is via `idea_id` (the idea carries its own `plan_id`); `save_post_content` does not take `plan_id`. **Angle lineage is separate and explicit** — `save_post_content` takes `brief_id`, resolved in Step 1c and passed on every save (Step 8).
...
Hold the chosen brief's **`id`** — it is passed to `save_post_content` as `brief_id` on **every** row you save this run (Step 8), recording the angle each section was written from. It is the id the operator gave you and you just resolved here: **never** re-derive it, infer it, or substitute "the idea's only brief" / the most recent / the first approved one.

Hold, too, the chosen brief's **`angle_label`** and its five narrative fields — **`hook_direction`**, **`core_message`**, **`why_now`**, **`story_moment`**, **`cta`** — as **the angle anchor** for every section produced this run. Use **only** this one brief; never pool across the idea's other briefs.

> **Server note (multi-angle is LIVE):** an idea routinely carries **several** approved angle briefs, each with a populated `angle_label` — `list_briefs(idea)` returns them all. This skill writes from **exactly ONE** of them: the brief whose `id === brief_id`, the id the operator invoked you with. Selecting the wrong row here mis-stamps every section this run produces, so filter on `brief_id` and nothing else — never "the idea's only brief" (there rarely is one), never the most recent, never the first approved.
...
Call: save_post_content
  ...
  brief_id: <the chosen angle brief's id from Step 1c — the SAME id on every row, in EVERY section>
...
- `brief_id` — the chosen approved angle brief's id, held from Step 1c: **the id the operator invoked you with**, which you already resolved via `list_briefs` and filtered to that one row. Pass it on **EVERY** save, in **ALL FOUR** sections (`copy`, `headline`, `description`, `image_content`) — the row then records which angle it was written from, which is true by construction (this run wrote every section from that one brief). **Never** derive, infer, or guess it: not "the idea's only brief", not the most recent, not the first approved.
...
> **Why `brief_id` must be passed explicitly — do NOT "clean this up" as redundant.** Omitting it does **not** leave the row's angle empty. The server binds a `brief_id` for **ad** content too — **by INFERENCE**, picking one of the idea's briefs for you. So an omitted argument does not produce a null you could later detect and repair; it produces a **stamped angle you did not choose**.
>
> **And the inference is now a coin toss.** An idea routinely carries **several approved angle briefs** (multi-angle briefs are live — see the Step 1c server note), so the brief the server infers can easily be the **wrong** one. Verified on live data: every ad `content` row on an idea with five approved briefs came back stamped with a single inferred `brief_id`, written by a version of this skill that never passed one.
>
> **A wrong stamp is silent and undetectable downstream.** `/ssc.image` resolves *"the approved copy for **THIS** angle"* by filtering `list_post_content` rows on `brief_id`. A mis-inferred stamp still matches that filter — nothing errors, nothing looks empty — so the visual gets grounded in copy written for a **different angle**, and the image tells the wrong story at fal-credit cost. A null would at least be visible; a wrong value is not.
>
> **`save_post_content` documents that "an explicit value always wins"** — which is precisely why passing it explicitly is never wrong, never a no-op, and never redundant. Overriding the inference with the operator's actual choice is the whole point of the argument.
>
> **Never derive it, never infer it.** The value is the `brief_id` the operator invoked you with and you resolved via `list_briefs` in Step 1c — never "the idea's only brief", never the most recent, never the first approved.
...
- **Saved, not presented.** For the single active section, per-section DRAFT `content` rows via `save_post_content(channel='ad', idea_id, brief_id, section, body, score, comment)` — the section's count (default 5 headlines / 5 copies / 5 descriptions / 5 image-content sets), every saved variation rated ≥4 with a Vietnamese comment. Saved immediately after scoring; there is no in-chat candidate presentation or revise loop. Saving persists drafts; it is NOT approval/selection.
- **Every saved row records its angle.** Each row carries the run's `brief_id` — the chosen approved angle brief it was written from — in **all four** sections (`copy`, `headline`, `description`, `image_content`), so a downstream consumer (e.g. `/ssc.image`) can resolve "the approved content for THIS angle" by filtering on `brief_id`.
...
- **Brief lineage is persisted (hard rule).** Every `save_post_content` call passes `brief_id` — the operator-supplied, `list_briefs`-resolved id from Step 1c — on **all four** sections (`copy`, `headline`, `description`, `image_content`), never derived/inferred/guessed. Omitting it does **not** null the field: for `ad` content the server **infers** a `brief_id`, picking one of the idea's briefs — and an idea routinely carries **several approved angle briefs**, so that inference can pick the **wrong** angle. The row is then stamped with an angle it was not written from, **silently** — `/ssc.image` filters `list_post_content` on `brief_id` to find "the approved copy for THIS angle", a mis-inferred stamp still matches, and the visual is grounded in the wrong angle at fal-credit cost with nothing downstream able to detect it. `save_post_content` documents that **an explicit value always wins** — so passing it is never redundant, and must not be removed.
