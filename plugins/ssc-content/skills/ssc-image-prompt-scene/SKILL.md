...
It returns the brief's creatives grouped by `layer`, each with `id`, `layer`, `status` (`draft` | `approved` | `discarded`), its resolved gallery item (caption/tags/url), and `generation_prompt`. A **selected** parent = a creative of that layer with **`status='approved'`** (the operator's "selected-for-next" curation act; `draft` = not yet selected, `discarded` = ignore entirely).
...
1. **The two parents' scene prompts** — read the selected background's and the selected subject's `generation_prompt` from the Step 2 `list_creatives` result (fall back to `list_creative_prompts(brief_id)` for the active layer prompt only when a creative's `generation_prompt` is absent). From them, extract the **light direction, time-of-day, perspective/lens, scale, and palette** the two must share — this is what makes the composed Scene read as **one photograph**, not a person pasted onto a mismatched set.
