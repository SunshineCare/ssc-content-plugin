...
**Is the subject stage still open?** If a `status='approved'` **subject** creative already exists, this stage is done — STOP (Vietnamese): stage subject đã có ứng viên được chọn; chạy lại `/ssc.image-prompt <brief_id>` để sang bước Scene. **Staleness (warn, never block):** if a later stage (scene/composite/text) already has a selected candidate, add — *đổi ảnh người mẫu ở bước này sẽ khiến các bước sau (đã dựng trên ảnh hiện tại) bị lỗi thời, cần dựng lại.* Also read `list_creative_prompts(brief_id)` for an existing `subject` prompt row (hold its `version` for the `revise` guard): a saved subject prompt **with no `revise` note** → STOP (Vietnamese): prompt subject đã lưu — hãy **Generate + chọn 1 ứng viên** trong ImageStudio, rồi chạy lại. A saved subject prompt **with a `revise` note** → go to the revise path (Step 8).
...
When you generate the person on the **FLUX.2 ladder** (no identity reference), FLUX.2 renders described appearance, wardrobe, pose, and expression faithfully — so describe them **precisely and in order**: **age & build → face & hair → wardrobe (matched to the selected background's palette) → pose & framing → expression → light (direction / quality / colour temperature, matching the scene) → lens**. Keep the ground **simple and positively described** for a clean cut-out at Scene. FLUX.2's stronger adherence means the no-negation / no-named-copy rules still hold in full — a named or negated string is *more* likely to be drawn. *(The identity path — `fal-ai/flux-pulid` / `fal-ai/flux-general` — is unchanged: the reference photo governs the face, and `idWeight` its strength.)*
