...
**Is the subject stage still open?** If a `status='approved'` **subject** creative already exists, this stage is done — STOP (Vietnamese): stage subject đã có ứng viên được chọn; chạy lại `/ssc.image-prompt <brief_id>` để sang bước **Composition (Ghép)**. **Staleness (warn, never block):** if a later stage (composition / edit / text) already has a selected candidate, add — *đổi ảnh người mẫu ở bước này sẽ khiến các bước sau (đã dựng trên ảnh hiện tại) bị lỗi thời, cần dựng lại.* Also read `list_creative_prompts(brief_id)` for an existing `subject` prompt row (hold its `version` for the `revise` guard): a saved subject prompt **with no `revise` note** → STOP (Vietnamese): prompt subject đã lưu — hãy **Generate + chọn 1 ứng viên** trong ImageStudio, rồi chạy lại. A saved subject prompt **with a `revise` note** → go to the revise path (Step 8).
...
**Revise path (`revise: <note>` supplied).** The active layer is `subject`. **Rewrite** the saved `subject` prompt applying the note — still obeying every prompt rule (never name copy/contents, never negate, keep the person alone + grounded in the intended scene) — and re-save with the optimistic-concurrency guard:
