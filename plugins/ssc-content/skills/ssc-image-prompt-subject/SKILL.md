...
**Subject has no required prior stage.** Subject is the **optional anchor**; it **waits on nothing upstream** (a Scene backdrop is optional, and the **Compose step runs AFTER you**). The person is generated ALONE on a simple, neutral ground so she **cuts out cleanly** when the **Compose step** composes her into a full scene — but her **outfit, wardrobe, styling, colour palette, and light** are grounded in the brief's **INTENDED scene** (its `story_moment` + the persona's world), because at this point there is **no background image yet to match**. The downstream **Compose step builds the scene AROUND her** and matches its light, perspective, scale, and palette **to her**, so the composite reads as **one photograph**, never a person pasted onto a mismatched set.
...
The chain: **Scene (optional backdrop) → Subject (you, optional anchor) → Compose → Edit (optional, repeatable) → Text.** The **Compose** step (`ssc-image-prompt-compose`, layer `model`) is the step that composes/builds the scene around this subject — Subject still saves only `layer:'subject'`.
...
**Is the subject stage still open?** If a `status='approved'` **subject** creative already exists, this stage is done — STOP (Vietnamese): stage subject đã có ứng viên được chọn; chạy lại `/ssc.image-prompt <brief_id>` để sang bước **Compose (Ghép)**. **Staleness (warn, never block):** if a later stage (compose / edit / text) already has a selected candidate, add — *đổi ảnh người mẫu ở bước này sẽ khiến các bước sau (đã dựng trên ảnh hiện tại) bị lỗi thời, cần dựng lại.* Also read `list_creative_prompts(brief_id)` for an existing `subject` prompt row (hold its `version` for the `revise` guard): a saved subject prompt **with no `revise` note** → STOP (Vietnamese): prompt subject đã lưu — hãy **Generate + chọn 1 ứng viên** trong ImageStudio, rồi chạy lại. A saved subject prompt **with a `revise` note** → go to the revise path (Step 8).
...
- **Wardrobe / styling / palette / light — grounded in the brief's INTENDED scene** (Step 4): choose an outfit tone and a light quality (direction, softness, colour temperature) that **suit the world the ad will live in** (the persona's home / the `story_moment`), so the Compose step can build a **coherent scene around her**. Keep the light natural and specifically describable so the downstream compose can match to it. Same lens register (e.g. 50mm, eye level, shallow depth of field) the scene will carry.
- **Simple neutral ground** — a clean, even, softly-lit plain backdrop so she cuts out cleanly when the Compose step composes her in.
...
> Next: mở ImageStudio → **Generate** rồi **chọn 1 ứng viên subject** (khoá gương mặt + tư thế). Sau đó chạy lại `/ssc.image-prompt <brief_id>` để sang bước **Compose (Ghép)** — bước đó sẽ dựng cả khung cảnh **quanh người mẫu đã chọn** (có thể dựng thêm một ảnh **Scene (Bối cảnh)** trước để ghép lên). Muốn sửa prompt này: chạy lại với `revise: <ghi chú>`.

(Subject là bước **tùy chọn** — nếu không cần khoá một gương mặt cụ thể, bạn có thể bỏ qua và sang bước **Compose (Ghép)** bằng `/ssc.image-prompt <brief_id> stage: compose` (hoặc dựng một ảnh **Scene (Bối cảnh)** trước bằng `stage: scene`). When no identity ref resolved, add: *gắn ảnh người mẫu thật trong studio nếu muốn khoá đúng gương mặt.*)
...
- **The optional Subject step; next is Compose.** Subject has **no required prior-stage precondition**; its only gates are the three brief/idea gates in Step 1. After a subject candidate is selected the pipeline advances to **Compose** (`ssc-image-prompt-compose`, layer `model`), which composes the scene around this anchor. Subject is optional — the operator may skip straight to Compose (or build a Scene backdrop first).
- **This stage saves `layer:'subject'` — only that.** The person is `subject`; the **Compose step** (`layer:'model'`, studio label "Compose" / *Ghép*, `ssc-image-prompt-compose`) builds the scene around this anchor and is a **separate** skill — the `model` layer is the **Compose step's job**, so Subject never saves it. The **Scene step** (`layer:'background'`, studio label "Scene", `ssc-image-prompt-background`) is a separate optional backdrop skill. Never save `layer:'product'` (upload-only, rejected).
- **Person ALONE, the anchor for the scene (hard rule).** Generate the woman on a simple neutral ground for a clean cut-out, with wardrobe / styling / palette / light grounded in the brief's **intended scene** (its `story_moment` + the persona's world) — there is no background image yet to match. The Compose step composes the scene AROUND her and matches its light/scale/perspective/palette to her, so the result reads as one photograph.
