---
name: ssc-image
description: The VISUAL producer of the Cambridge Diet Vietnam ad-production workflow — a STATE-DRIVEN, per-step stepper that SPENDS fal credits by calling the generate tools (the credit-spending counterpart of the propose-only ImageStudio prompt authors, the visual sibling of ssc-ads-writer, and the still-image sibling of /ssc.video). Anchored to ONE approved angle brief. The 5-step pipeline (ALL steps OPTIONAL): Step 1 — Scene (tool generate_background, layer key `background`) a PURE text-to-image backdrop with NO subject/product anchors and NO reserved zones — a brief-level backdrop; Step 2 — Subject (tool generate_subject, layer key `subject`) a standalone person generated ALONE, a brief-level anchor; Step 3 — Compose (tool generate_model, layer key `model`) the ANCHOR-GATED combine — REVIVED (was retired) — needs ≥1 anchor (a selected subject OR an approved product; a selected Scene ALONE does not satisfy the gate) — with a selected Scene it composes the anchor(s) ONTO that scene (scene = edit base), without a Scene it builds AROUND the anchor(s); a Kontext image-edit over control-capable models, control-source defaults to the product; Step 4 — Edit (tool compose_ad_visual, layer key `composite`) a GENERIC, repeatable prompt-to-edit over the chain tip; Step 5 — Text (tool generate_text_layer, layer key `text`) renders the approved headline over the chain tip. Lineage: parent = the nearest PREVIOUS selection, walking ['composite','model','subject','background'] (nearest-first, optional steps transparent) — a chain tip can be a `composite`, a `model` (Compose), a `subject`, or a `background` (Scene); Compose is the exception (it combines a selected Scene + subject + product); Text is NOT anchor-gated (Text-on-Scene allowed). A brief carries N PARALLEL chains; chain selection is never silent — ≥2 chains and no `chain` → STOP-and-ask; `new-chain` mints an ADDITIONAL chain. Scene + Subject + product are BRIEF-LEVEL anchor inputs to Compose, shared by every chain. THREE preconditions in order — the idea is channel='ad' + approved; brief_id is an APPROVED angle brief of it; ≥1 APPROVED copy exists for that brief (Text additionally needs an approved image_content headline). Authors the FULL prompt for every call; it reaches the engine VERBATIM under four prompt rules (never name the ad copy; never negate; one image per call; no baked-in text) with NO reserved-zone geometry. A CHAIN completes when it holds an approved `text`; the BRIEF is never complete, and choosing the hero image (set_cover) stays the operator's dashboard action. Propose-only on APPROVAL: saves DRAFT creatives, never approves, discards, sets a cover, reorders, publishes, or updates a budget. Operator-facing prose and persisted notes are Vietnamese; image prompts are free-form.
metadata:
  type: skill
  stage: produce
  ...
  section: ads
  capability: edit
  tools: [get_idea, list_briefs, list_content, get_knowledge, list_creatives, list_creative_prompts, list_gallery_media, generate_subject, generate_background, generate_model, compose_ad_visual, generate_text_layer]
---
...
|---|---|---|---|---|
| 1 | **Scene** (Bối cảnh) | `background` | `generate_background` | OPTIONAL — a **pure text-to-image** backdrop, brief-level; NO anchors, NO reserved zones |
| 2 | **Subject** (Người mẫu) | `subject` | `generate_subject` | OPTIONAL — a standalone person, a brief-level anchor candidate |
| 3 | **Compose** (Ghép) | `model` | `generate_model` | OPTIONAL, **ANCHOR-GATED** — combines the anchor(s) (± a Scene base) into the chain trunk |
| 4 | **Edit** (Chỉnh sửa) | `composite` | `compose_ad_visual` | OPTIONAL + **REPEATABLE** — a generic prompt-to-edit over the chain tip |
| 5 | **Text** (Tiêu đề) | `text` | `generate_text_layer` | renders the approved headline over the chain tip |

**All five layer KEYS are live.** `model` (**Compose**) is **REVIVED** — this skill produces it and **calls `generate_model`**. `background` is the **Scene** backdrop (pure text-to-image, no anchors); `composite` is **Edit**. There is no dormant / retired layer — Subject, Edit and Text simply never SAVE `layer:'model'` themselves *because that is the Compose step's job*, not because `model` is retired.

**Lineage is the `derived_from` chain — parent = the nearest PREVIOUS selection.** There are no `background_creative_id` / `scene_creative_id` FK columns to reason about — the tools resolve the parent by walking the `derived_from` provenance chain, **nearest-first, optional steps transparent**:

```
scene(opt) ┐
subject(opt)├─▶ compose ──parent──▶ edit ──parent──▶ edit … ──▶ text
product ────┘  (anchor-gated; ± scene base)
```

- **Edit and Text** parent onto the single selected candidate from the **nearest upstream step that has one**, walking `['composite','model','subject','background']`. So a chain **tip** can be a `composite`, a `model` (Compose), a `subject`, or a `background` (Scene).
- **Compose is the EXCEPTION** — it does NOT use the nearest-selection walk. It **combines** a selected Scene (`background`, the edit base) + a selected `subject` + an approved `product`.
...
- **Skip Scene** → Compose builds a new image **AROUND** the anchor(s) (no scene base).
- **Skip Subject** → Compose anchors on the **product** alone; with **neither** subject nor product Compose is gated OUT (STOP).
- **Skip Compose** → Edit / Text hang off the nearest approved trunk — an approved **Scene** or **Subject**.
- **Skip Edit** → Text hangs off the chain tip (an approved `composite`, else `model`, else `subject`, else `background`).

**A brief carries N PARALLEL chains — chain-scoped, and chain selection is never silent.** A chain trunk is an **approved `model` (Compose)** — **chain id = that creative's id** — and the operator may approve several. Because every step is optional and Text is not anchor-gated, a chain may instead root **directly on an approved Scene or Subject** (Compose skipped: Edit / Text hang off it). So "the Edit layer has an approved creative" tells you nothing about the chain you are advancing — another chain's edit may be what you are seeing. You therefore evaluate every state gate **inside ONE selected chain**, **NEVER** edit or text across chains, and **NEVER** pass another chain's tip as `from`. No chain trunk yet → the run is chain CREATION (Compose, or a Scene/Subject the operator elects to build on). Exactly one chain → that chain, unambiguously (resolve it, do not ask). **More than one** and no `chain` argument → **STOP and ASK** which chain to advance. Guessing here is the silent-wrong-target bug this skill exists to avoid — and every generate call spends fal credits, so a STOP-and-ask is always cheaper than a wrong image.

**Scene, Subject and product are BRIEF-LEVEL anchor inputs to Compose, shared by every chain.** A `scene` is a pure backdrop generated by this skill (Step 1, optional); a `subject` is a standalone person generated by this skill (Step 2, optional); a `product` is the **real packaging photograph**, uploaded on the dashboard (never generated, never uploaded by you). All three are anchor references the **Compose** step consumes — they belong to no chain, so one approved scene / subject / product serves every chain.
...
- `stage` — explicitly select a step: **`scene`** (alias `background`, Step 1) · **`subject`** (Step 2) · **`compose`** (alias `model`, Step 3) · **`edit`** (alias `composite`, Step 4) · **`text`** (Step 5). Omit it and Edit is triggered by an `edit:` note; anything outside those five tokens → STOP (`invalid_input`).
...
- `chain` — a **chain-root creative id** naming the **approved trunk that roots the chain to advance** (an approved `model` Compose, or — on a Compose-skipped chain — an approved `background`/`subject` the operator builds Edit / Text on). A brief carries N parallel chains; this says which. No chain trunk yet → not required (the run is chain creation); exactly one → not required (unambiguous); **≥2 and no `chain` → STOP and ASK**; a `chain` that is not a valid trunk of this brief → **STOP** (`invalid_input`) naming the valid roots.
- `new-chain` — mint an **ADDITIONAL** chain: run **Compose** to generate 3 fresh Compose candidates **even though the brief already has an approved chain trunk**. This (like `stage: compose`) is how Step 3 runs once a chain exists — absent an explicit Compose trigger, a plain re-invocation never burns credits minting trunks nobody asked for. `new-chain` together with `chain` is **contradictory** → STOP (`invalid_input`).
- `scene` — an **approved `background` (Scene) creative id** naming which backdrop the Compose step edits onto. Only needed when the brief has **more than one** approved Scene (with exactly one, it is used without asking; with none, Compose builds around the anchor(s) with no scene base). An invalid id → **STOP** (`invalid_input`) listing the approved Scenes.
- `subject` — an **approved `subject` creative id** naming which anchor person Compose should be built around. Only needed when the brief has **more than one** approved subject (with exactly one, it is used without asking). An invalid id → **STOP** (`invalid_input`) listing the approved subjects.
...
There is **no `layout_hint` / `hard_paste`** any more: `compose_ad_visual` is a generic edit and takes neither — product placement direction lives in the `edit:` change text. There is **no `uploaded_media_id` / `uploaded_media_ref`** (a real model photo is placed via an operator dashboard upload), **no `image_content_id`**, and **no `n`** parameter.
...
`list_creatives` returns `creatives[]`, each with `id`, `layer` (`subject` | `background` | `model` | `product` | `composite` | `text`), `status` (**`draft` | `approved` | `discarded`**, all three live), and its joined **`media`** pool item — including **`media.provenance`** (`{ prompt, model, derived_from }`, the frozen record of how the image was made) and the pool item's own id (its **galleryItemId** / `ref`, the value the generate tools take as `scene` (`from`) / `subject` / `product` / `from`). *(There is no `generation_prompt` column on the creative — the prompt lives on `media.provenance.prompt`.)*

**Chain membership is derived from the `derived_from` lineage — no server-side `chain_id`:**

| Layer | Lineage | Chain membership |
|---|---|---|
| `background` (Scene) | `derived_from:[]` | **BRIEF-LEVEL backdrop anchor**, in no chain by default — an anchor input to Compose, shared by the whole brief. May become a chain ROOT only when the operator builds Text / Edit directly on it (Compose skipped). |
| `subject` (Subject) | `derived_from:[]` | **BRIEF-LEVEL person anchor**, in no chain by default — an anchor input to Compose. May likewise become a chain ROOT when Edit / Text hang off it directly. |
| `model` (Compose) | `derived_from:[scene? + subject? + product?]` | An **APPROVED** Compose **IS a chain ROOT** — **chain id = that creative's id**. A `model` **draft** is a **candidate root**, in no chain yet: *approval curates it into one*. A valid `from`/parent for Edit / Text. |
| `composite` (Edit) | `derived_from:[parent]` | Belongs to the chain reached by walking `derived_from` back through its parent to the trunk. |
...
- `ROOTS` — every **approved** `model` (Compose), **plus** any approved `background`/`subject` a `composite`/`text` already hangs off (a Compose-skipped trunk). `|ROOTS|` = **how many chains this brief has**.
- `CANDIDATE_ROOTS` — every **draft** `model` (Compose): candidate roots awaiting selection.
- `approved_scenes` — every **approved** `background` (Scene); `scene_drafts` — every **draft** `background`.
...
| **A2** | `stage` is present and is not one of `scene`/`background`, `subject`, `compose`/`model`, `edit`/`composite`, `text` | **STOP** (`invalid_input`). Generate nothing. |
| **A3** | `chain` **and** `new-chain` both supplied | **STOP** (`invalid_input`) — contradictory. Generate nothing. |
| **A4** | `scene` / `subject` / `product` supplied and it is **not** the id of an approved `background` / `subject` / `product` of this brief | **STOP** (`invalid_input`) — list the valid ones. Generate nothing. |
| **A5** | `chain` supplied and it is **not** a valid chain trunk of this brief (an approved `model`, or an approved `background`/`subject` serving as a trunk) | **STOP** (`invalid_input`) — list the brief's valid chain roots (id + one-line gist of `media.provenance.prompt`). Generate nothing. |
...
| **A8** | **`stage: compose`** OR **`new-chain`** | **STEP 3 COMPOSE — chain CREATION** (anchor-gated). No chain selected → **Step 7-compose** (a `revise` note → Step 8 case B). |
...
| **A12** | `|ROOTS| == 0` and no `stage`/`new-chain` | **STOP** — no chain trunk yet, nothing to advance. Guide the operator to start a step: `stage: scene` (bối cảnh), `stage: subject` (người mẫu), or `stage: compose` (ghép — cần ≥1 anchor). Generate nothing. |
...
>
> | chain (root Compose/Scene) | Nội dung | Tiến độ |
> |---|---|---|
...
**Compose chain-creation rules** (reached from **A8**: `stage: compose` / `new-chain`; the `model` pool is brief-level because a candidate root belongs to no chain). The **anchor gate** is resolved inside Step 7-compose:

| # | Condition | Action |
|---|---|---|
| **M1** | `CANDIDATE_ROOTS` non-empty (≥1 draft Compose), **no `revise`** | **STOP** — candidate Compose images await selection; **approve 1** in `/ad/<month>/<idea_id>` (that mints a chain) or discard. **No second batch.** |
| **M2** | `CANDIDATE_ROOTS` non-empty, **`revise` supplied** | → **Step 7-compose**, but **Step 8 case A** — rewrite ONE candidate's prompt, **ONE** `generate_model` call. Never a second batch of 3. |
| **M3** | `CANDIDATE_ROOTS` empty | → **Step 7-compose** — resolve + gate the anchors, then 3 fresh readings (a `revise` note → Step 8 case B, folded into all three). |

*(**A8** with a trunk already present mints an ADDITIONAL chain — it enters **Step 7-compose** directly: 3 fresh readings distinct from every existing trunk. `stage: compose` / `new-chain` is required on every run that mints trunks — it is never implied.)*

**In-chain rules** (reached from **A9**/**A10**, ONE chain SELECTED; the chain has some approved trunk by construction; the **tip** = the latest approved `composite`, else approved `model`, else approved `subject`, else approved `background`; `revise` / `edit` mean *of this chain*):
...
| **C3** | `edit: <change>` (or `stage: edit`) supplied, no pending composite draft | active step = **Edit** → **Step 7-edit** — ONE `compose_ad_visual` over the chain tip (a `revise` note also present → Step 8 case B, folded in). Repeatable. |
...
**Hold for the rest of the run:** the **selected chain's ROOT** creative id (the chain id you report); the resolved **chain TIP** (via the walk above) and **its galleryItemId** (the `from` for Edit / Text); the resolved brief-level **anchor(s)** (scene / subject / product galleryItemIds, for a Compose); and `|ROOTS|` for the Step 11 summary.

### Step 7-scene: Step 1 — a pure text-to-image backdrop (OPTIONAL, `generate_background`)

Reached only via **A6** (`stage: scene`). A Scene is a **complete backdrop** — a real place (a bright kitchen, a sunlit living room), rendered **pure text-to-image** from scratch. It takes **NO subject/product anchors** and **NO reserved zones** — it is a brief-level backdrop the **Compose** step later edits anchors onto. First branch on the brief-level scene pool:

- `scene_drafts` non-empty, **no `revise`** → **STOP**: a backdrop candidate is pending; **approve 1** (or discard) in `/ad/<month>/<idea_id>`, then re-invoke (`stage: compose`) to compose the anchor(s) onto it.
- `scene_drafts` non-empty, **`revise` supplied** → **Step 8 case A** — rewrite ONE pending scene's prompt, **ONE** `generate_background` call.
- `scene_drafts` empty → generate **three** distinct backdrop candidates (a `revise` note → Step 8 case B, folded into all three).

Author each prompt from Step 4 grounding: a **complete, filled** environment — the persona's world (her home, her light, her life stage) coherent with the brief's `core_message` and `story_moment` (meaning only — Rule 1). Three genuine readings (setting / time-of-day / staging), not three re-rolls. Obey the Step 5 rules (no baked-in text; positive description; never name the copy; no reserved voids).

```
Call: generate_background
  brief_id: <brief_id>
  prompt:   <the FULL backdrop prompt — reading 1>          # NO subject/product refs — pure text-to-image
  model:    <only if the operator supplied one; otherwise OMIT>
```

…then again with reading 2, and reading 3. Capture the creative ids, then **STOP** — the operator approves one, and that approved Scene becomes a brief-level backdrop the Compose step edits onto.

### Step 7-subject: Step 2 — a standalone person (OPTIONAL, `generate_subject`)

Reached only via **A7** (`stage: subject`). A subject is a **standalone person on a neutral ground**, generated ALONE — a **brief-level anchor candidate** (parentless), later attached as a reference by the **Compose** step. First branch on the brief-level subject pool:

- `subject_drafts` non-empty, **no `revise`** → **STOP**: a subject candidate is pending; **approve 1** (or discard) in `/ad/<month>/<idea_id>`, then re-invoke (`stage: compose`) to compose around it.
...
…then reading 2, and reading 3. Capture the creative ids, then **STOP** — the operator approves one, and that approved subject becomes an anchor the Compose step attaches.

### Step 7-compose: Step 3 — combine the anchors into the chain trunk (ANCHOR-GATED, `generate_model`)

**This step is chain CREATION.** An approved Compose is a chain ROOT, so it runs when the operator asks for it (`stage: compose`, or `new-chain` to mint an ADDITIONAL trunk even though a chain exists — read the existing trunks' `media.provenance.prompt` and make the three new readings **genuinely distinct** from every one of them).
...
> **ANCHOR GATE (hard).** Compose requires **≥1 anchor = a selected `subject` OR an approved `product`**. With **NEITHER** → **STOP**, generate nothing (Vietnamese): *Bước Ghép cần ít nhất một anchor — một người mẫu (subject) đã duyệt HOẶC một ảnh sản phẩm (product). Một bối cảnh (scene) không đủ. Hãy chạy `stage: subject` để tạo người mẫu, hoặc upload ảnh sản phẩm, rồi chạy lại `stage: compose`.* **A selected Scene ALONE does NOT satisfy the gate.**

**Role.** Compose is always a **Kontext image-edit** over **control-capable models** (flux-general, control-depth are available). With a **selected Scene** (`from`) → compose the anchor(s) **ONTO** that scene (scene = edit base). **Without a Scene** → build a new image **AROUND** the anchor(s) (the environment is depth the model builds around them). **Control-source defaults to the product** (for a control model, no operator override); a **manually-picked control-pane source rides as an extra reference**. The operator's `model` override still wins the model id.

Issue **three separate** `generate_model` calls, each carrying a **distinct** full scene prompt — three genuine readings of the same angle (different staging / composition / treatment), not three re-rolls. Each mints ONE DRAFT `model` creative and is a **candidate root**: approving one mints the chain.
...
### Step 7-edit: Step 4 — a generic edit over the chain tip (OPTIONAL, REPEATABLE, `compose_ad_visual`)

Reached via **C3** (`edit: <change>` / `stage: edit`, no pending edit). `compose_ad_visual` is a **generic Kontext reference-edit** — it applies the operator's "what to change" over the **chain tip** (this chain's latest approved `composite`, else the next approved trunk in the walk — `model`, else `subject`, else `background`). Product placement is **one kind** of edit; there is no dedicated product step.
...
Reached via **C7** (`stage: text`, an approved `image_content` headline exists). Render the **exact** approved headline (the `image_content` body from Step 3) onto the chain tip. The renderer (Ideogram, with a deterministic diacritic-safe overlay fallback) writes the string **verbatim**, so Vietnamese diacritics stay intact. Text is **not anchor-gated** — the tip may be a `composite`, a `model` (Compose), a `subject`, or a `background` (Text-on-Scene is allowed).
...
- **Scene** (`generate_background`, no anchors) → **FLUX.2 text-to-image** (default `fal-ai/flux-2/klein/9b`, ladder-steppable). NEVER Kontext.
- **Subject** (`generate_subject`) → text-to-image / identity model.
- **Compose** (`generate_model`) → **Kontext image-edit** (`fal-ai/flux-pro/kontext`) / control-capable models (flux-general, control-depth).
- **Edit** (`compose_ad_visual`) → Kontext Pro.
...
| `no_approved_parent` | Compose / Edit / Text attempted with no approved parent (no approved scene base / anchor / chain tip) | **STOP** — duyệt anchor/tip của **chuỗi này** trước (một Scene/Subject/Compose, hoặc một edit) |
...
**Step produced:** <scene | subject | compose (Ghép) | edit | text>
**Built on:** <"— (Scene: bối cảnh độc lập, text-to-image) | (Subject: người mẫu độc lập)" | "anchor: subject <id> + product <id> (± scene base <id>, Kontext) | anchor: product <id>, dựng quanh anchor" | "chain tip <id> (Compose | edit)" | "chain tip <id> + tiêu đề đã duyệt">
...
- after **scene**: `Next: mở /ad/<month>/<idea_id> → duyệt 1 bối cảnh (scene), rồi chạy lại /ssc.image <idea_id> <brief_id> stage: compose để mình ghép anchor (người mẫu/sản phẩm) lên bối cảnh đã chọn.`
- after **subject**: `Next: mở /ad/<month>/<idea_id> → duyệt 1 người mẫu (subject), rồi chạy lại /ssc.image <idea_id> <brief_id> stage: compose để mình ghép quanh người mẫu đã chọn.`
- after **compose** (chain creation): `Next: mở /ad/<month>/<idea_id> → duyệt 1 ảnh Ghép — việc duyệt chính là mint root cho chuỗi (hoặc chạy lại với revise: <ghi chú> để sửa prompt). Rồi chạy lại /ssc.image <idea_id> <brief_id> chain: <root vừa duyệt> — thêm edit: <mô tả> để chỉnh sửa, hoặc stage: text để render tiêu đề.`
...
- **Saved, not presented.** DRAFT `creative` rows via the BrandOS tools (`generate_background` → `layer='background'` (Scene), 3 × one per call, pure text-to-image; `generate_subject` → `layer='subject'`, 3 × one per call; `generate_model` → 3 × `layer='model'` (Compose), one per call, each a candidate chain root; `compose_ad_visual` → 1 × `layer='composite'` (Edit) in the selected chain; `generate_text_layer` → 1 × `layer='text'` in the selected chain; `product` is brief-level and upload-only, never produced here). Saved immediately; **no in-chat candidate presentation and no in-chat revise loop**. Saving persists drafts; it is **NOT** approval — and on `model`, approval also **curates a candidate into a chain root**.
- **One step of ONE chain per invocation.** The operator approves (or discards) in the dashboard and re-invokes for the next step of that chain — or re-invokes with `revise: <note>` to rewrite the active step, `edit: <change>` for another edit, `stage: scene` / `stage: subject` / `stage: compose` / `stage: text` for those steps, or `new-chain` to start another parallel chain.
...
- **No baked-in text upstream.** Every Scene / Subject / Compose / Edit visual is word-free (achieved through positive description); the words appear only at the Text step.
...
- **Propose-only on APPROVAL (hard rule).** Never call any tool that changes approval or lifecycle state — never `approve` (the ONLY gated promotion; the approval hook denies it to agents), never use `edit` to demote / unapprove / **discard** a creative (discarding is the operator's call), never `save_creative_prompt`, `upload_creative` / `confirm_creative_upload` / `select_gallery_creative`, `set_cover`, `reorder_gallery`, any publish tool, or `update_budget`. Save DRAFT `creative` rows and STOP. **Saving is not approving** — and approving a Compose also **curates it into a chain root**. None of the forbidden tools appears in this skill's `tools:` list.
- **The 5-step pipeline (hard rule).** `background` "Scene" (opt, `generate_background`, **pure text-to-image**, no anchors) → `subject` "Subject" (opt, `generate_subject`, standalone person) → `model` "Compose" (opt, **anchor-gated**, `generate_model`, combines anchor(s) ± a Scene base) → `composite` "Edit" (opt + repeatable, `compose_ad_visual`, generic prompt-to-edit) → `text` (`generate_text_layer`). All five steps are **OPTIONAL**. The `model` (Compose) step is **REVIVED** — `generate_model` IS called; there is no retired / dormant layer. Product placement is one kind of Edit, not a dedicated step.
- **Optional-step transparency (hard rule).** Every step derives its state from what exists, never from a fixed predecessor: skip Scene → Compose builds around the anchor(s); skip Subject → Compose anchors on the product (neither → gated OUT); skip Compose → Edit/Text hang off the nearest approved trunk (a Scene or Subject); skip Edit → Text hangs off the chain tip. `buildLayers`-style derivation, not assumed predecessors.
- **Single-role Scene (hard rule).** `generate_background` is now **pure text-to-image** — it attaches **NO** subject/product refs and reserves **NO** zones. It renders a complete, filled backdrop the Compose step later edits anchors onto. (The old dual-role "Full image" auto-resolution is GONE.)
- **Anchor-gated Compose (hard rule).** `generate_model` requires **≥1 anchor = a selected `subject` OR an approved `product`**; a selected Scene ALONE does not satisfy the gate → STOP. With a selected Scene (passed as `from`, the edit base) → compose the anchor(s) ONTO it; without → build AROUND the anchor(s). Always a Kontext image-edit over control-capable models; **control-source defaults to the product**; a manually-picked control-pane source rides as an extra reference. The operator's `model` override still wins the model id.
- **Generic repeatable Edit (hard rule).** `compose_ad_visual` applies the operator's `edit: <change>` over the **chain tip** (latest approved `composite`, else the next approved trunk in the walk), `derived_from`-chained so edits stack edit-on-edit; it is OPTIONAL (skip it) and REPEATABLE. Product placement, lighting, clutter, composition — all are edits.
- **Chain-scoped state, lineage = nearest previous selection (hard rule).** A brief carries **N parallel chains**, each rooted at an **approved `model` (Compose)** — or, Compose skipped, directly on an approved `background`/`subject` a `composite`/`text` hangs off. Parent = the nearest PREVIOUS selection, walking `['composite','model','subject','background']` (nearest-first, optional-transparent) — so a chain tip / valid `from` can be a `composite`, a `model`, a `subject`, or a `background`. Compose is the EXCEPTION (it combines a selected Scene + subject + product). There is no `background_creative_id` / `scene_creative_id` FK and no server-side `chain_id`. `approved(L)` / `has_drafts(L)` are computed **only inside the selected chain**. Never advance a chain on another chain's creative; **never pass another chain's tip as `from`**. A non-discarded `composite`/`text` whose lineage does not reach an approved trunk is **unassignable** → report and STOP. `discarded` is a real third status: ignored for state, its prompt only a chain-scoped revise base.
- **Chain selection is explicit — never silent (hard rule).** No chain trunk → the run is chain CREATION (Compose, or a Scene/Subject the operator builds on). Exactly one → that chain, resolved **without asking**. **≥2 and no `chain` → STOP and ASK**, listing every chain (root id + one-line gist of `media.provenance.prompt` + progress). An invalid `chain` → **STOP** (`invalid_input`). `chain` + `new-chain` → **STOP** (contradictory). **Never pick a chain by any heuristic.**
- **`stage: compose` / `new-chain` is the only way to mint a trunk.** A Compose run generates 3 fresh candidate roots and STOPs for approval; `new-chain` mints an ADDITIONAL one distinct from every existing trunk. Required on every run that mints trunks — never implied by a plain re-invocation.
...
- **One concept + one angle brief + ONE chain (or one brief-level Scene/Subject/Compose batch) per invocation.** Re-invoke per brief, per chain, per step.
- **Operator-facing prose and persisted notes are Vietnamese**; image-model prompts are free-form.
- Requires the `edit` capability — for the generate tools AND for the `list_creatives` / `list_creative_prompts` / `list_gallery_media` reads. Only the concept / brief / copy / knowledge reads (`get_idea` / `list_briefs` / `list_content` / `get_knowledge`) are satisfied by `view`. Approving a saved draft (which also **mints a chain root** on a Compose), discarding one, uploading the real product, and choosing the hero image (`set_cover`) are the operator's dashboard actions. An `edit`-holding operator still refused by a generate tool (`insufficient role`) is a **server-side permission** — report it and STOP; never retry around it or reach for a provider API.
