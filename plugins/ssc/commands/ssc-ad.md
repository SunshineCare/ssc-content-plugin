...
- **Section** (`section`) — one of `copy` | `headline` | `description` | `image_content` (`/ssc-ad <brief_id> copy`, `… headline`, …). Names what to produce/revise this invocation. **All four are valid explicit values and an explicit name always wins over the auto-pick** — naming **`copy`** targets `copy` **including when a copy is already approved**, which is how you get a fresh batch of copy variations after the first approval (non-destructive: the write path only INSERTS drafts). Omit to auto-pick the next open text section (`copy` first if not yet approved, else the first of `headline → description → image_content` without an approved row). `copy` remains the mandatory **cold start** — before it is approved, no other section can be produced whatever this argument says.
