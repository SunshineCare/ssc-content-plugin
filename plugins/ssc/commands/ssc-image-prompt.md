---
argument-hint: '<brief_id> [scene|subject|composition|edit|text] [setup: <n>|rewrite|revise: <note>]'
description: >-
  ...
  Scene → Subject → Composition → Edit → Text. The only image path Cowork has, and it
  is zero-credit — the operator clicks Generate in the ImageStudio dashboard, which is
  what spends fal credits.
metadata:
...
- **`text`** (Tiêu đề, backend layer `text`) — renders the **exact approved Vietnamese
  headline** over the chain tip.
