...
| **Measure** | ungated | The **conversion lens**: reads this plan's ingested ad performance (plus the boosted page posts), grades `paid_only` on its declared tier's locked KPI (L2 never on cost-per-purchase) and `boosted` as its own boost class with **no tier**, excludes `organic_only` (that is `/ssc.post-plan`'s engagement lens) → writes the `retrospective` (carried into next month's Focus) | — (ungated; closes the loop) |
