...
- **Decision fidelity** — do `hook_direction` / `core_message` / `why_now` plainly read as the SAME route, stage, and anchor Step 3 diagnosed and Step 4 was supposed to write to — and does `angle_label` name that same anchor? A field that quietly drifts to a different route/stage/anchor than what was decided caps low here even if the drift reads well on its own.
