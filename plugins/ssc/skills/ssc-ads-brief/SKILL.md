...
- **`why_now`** — the ad-set's audience-stage/timing rationale: state the **awareness stage diagnosed in Step 1d** and the framework's rationale for why *this* angle type is the one that lands on that stage, combined with the plan period and anchored to **this angle's** trigger point where one applies.

  > **`why_now` is a CONTRACT, not just prose — `ssc-ads-writer` reads it back.** The writer does **not** re-diagnose the awareness stage or re-derive the plan period; it takes both from this field so the copy realizes the angle you approved rather than drifting onto a different one. So `why_now` must **explicitly name three things in plain Vietnamese**: (1) the **awareness stage**, (2) the **angle type** it implies (Problem / Solution / Comparison / Proof / Curiosity), and (3) the **plan period `YYYY-MM`** you resolved in Step 1b. Leave any of them out and the writer silently falls back to its own diagnosis and to `created_at` — which is exactly the divergence this field exists to prevent. Write them plainly enough to be read off at a glance. Take the stage→angle reasoning from the live `ad/awareness-framework` mapping — do not restate a remembered tier→stage table. If `build_spec` was unavailable (Step 1b), derive from the idea's tags + plan period alone, note the stage was inferred from the `layer` tag, and say so.
...
- **`why_now` is a downstream CONTRACT (hard rule).** `ssc-ads-writer` reads the **awareness stage**, the **angle type**, and the **plan period `YYYY-MM`** back out of this field instead of re-deriving them — that is what stops the copy drifting onto a different angle type or a different ad set than the one this brief was written against. All three must be named explicitly in the `why_now` prose. Omitting any of them silently degrades the writer to its own diagnosis and to `idea.created_at`.
