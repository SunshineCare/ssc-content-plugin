...
Hold the resulting **fitting-persona set** — each with her held anchors, her `Tránh` list, and her taxonomy `code` — forward into Step 2 onward. Also hold `ad/cta-catalog` (loaded in 1c) and, only when a founder/story-led angle is in play for some persona (`programme/kieu-my-story` — load it now if any fitting persona's connecting anchor looks story/confession-shaped; it is the sole source for any Kiều My personal scene, never invented).
...
- **`cta`** — a soft, compliant CTA phrasing from `ad/cta-catalog` appropriate to this angle's stage.
...
- **Structurally append-only (hard rule).** `delete` and `edit` are **NOT** in this skill's `tools:` list. `tools:` is exactly `[get_idea, get_channel_plan, get_knowledge, list_taxonomies, list_briefs, save_brief]` — five reads and one appending insert.
