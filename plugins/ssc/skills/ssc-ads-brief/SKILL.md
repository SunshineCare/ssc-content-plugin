...
idea. It does **not** pick a **lead type** — and that omission is a decision, not
a gap.
...
**The MECHANISM is carried, never authored.** Per `craft/doctrine` §2 the mechanism
lives on the **idea** — one per subject, resolved once at Ideate, inherited by
every angle beneath it. Every angle you write is written **to** it: consistent
with it, and leaving the writer able to hit the mechanism beat. No angle restates
it as a field of its own, paraphrases it into `core_message`, sharpens it,
softens it, or offers this persona's own competing mechanism — one subject
sprouting contradictory mechanisms across its personas is precisely what the
idea-level home prevents. An angle that genuinely cannot be written to the
subject's mechanism is a **misfit angle**: drop it, don't re-mechanise it.
...
- `idea.mechanism` — **the subject's mechanism, carried, never authored here.** It is
  resolved once at Ideate (a subject is not approvable without one) and inherited by every
  angle you write beneath it. Hold it and read `craft/doctrine` §2 live for what writing *to* a
  mechanism means; it constrains every `core_message` in Step 4 and is scored in Step 5.
  **Never write it, never patch it onto the idea, never vary it per persona.** If a legacy
  subject approved before this model carries **none**, do **not** stop and do **not** invent
  one: proceed, drop the mechanism-fidelity criterion for this run, and **name the absence
  plainly** in the Step 7 summary as a gap for the operator — the doctrine binds approvals
  made after it landed, not the ones already in the book.
...
Call: get_knowledge
  paths:
    ...
    - craft/doctrine            # §1 the chain; §2 the mechanism — what carrying one means (Step 4)
...
- **`craft/doctrine` in `missing`** — no live definition of the chain or of what writing to a mechanism means: STOP.
...
- **Confirm the angle can be written to the subject's mechanism** (`idea.mechanism`, Step 1). A candidate whose argument only works by supplying a *different* mechanism is a misfit — drop it rather than re-mechanising the subject. (Skip this check only when the subject is a legacy row carrying none; then say so in Step 7.)
...
- **`core_message`** — one clear Vietnamese sentence: the subject sharpened to this persona's anchor. Must serve **her** emotion-audit cluster (Step 1e) and, at Cambridge's sophistication position, carry mechanism and/or identification rather than a bare benefit claim. **It is written TO the subject's mechanism, never as a restatement of it** — consistent with `idea.mechanism`, leaving the writer able to hit the mechanism beat, and never paraphrasing, sharpening, softening or replacing it with this persona's own. Read `craft/doctrine` §2 live for what that distinction is; do not restate the mechanism as a sixth field.
...
- **Mechanism fidelity** — is this angle written **to** the subject's mechanism (`idea.mechanism`), consistent with it and leaving the writer able to hit the mechanism beat? **Any angle that restates, paraphrases, sharpens, softens, replaces or contradicts the subject's mechanism caps at 2 — a hard cap.** The mechanism is the idea's, inherited, never re-authored per persona. Skip this criterion only on a legacy subject carrying none (Step 1), and say so in Step 7.
...
**Never pass a lead, and never pass a mechanism.** The brief's declared identity is persona × route × anchor + `awareness_stage` + `target_layer_term_id` — **no lead type**, because the writer picks that per asset from the stage's admitted set. And **no mechanism field**: the mechanism lives on the `ideas` row, is inherited, and is written *to* by `core_message` — never copied down onto the brief, where it could drift out of agreement with the subject it came from.
...
- **mechanism-drifted** — its `core_message` restates the subject's mechanism, or supplies a competing one, instead of writing to `idea.mechanism`;
...
**Target subject:** <idea_id> — status approved
**Mechanism:** <the subject's mechanism, carried unchanged — every angle below is written TO it; none restates or varies it> — or "absent (legacy subject approved before the doctrine); mechanism fidelity not scored this run, and none was invented."
...
**Persisted:** <N> NEW DRAFT brief(s), appended alongside the existing <M> — one row per angle, each with its own `persona_term_id`/`route_term_id`/`target_layer_term_id`/`awareness_stage` plus `angle_label`/`brief_id`. **No lead type and no mechanism field was written on any row** — the lead is the writer's per-asset choice, the mechanism stays on the subject. `audience_intent` was not set on any row. **No APPROVED brief was touched.**
...
- **The subject's MECHANISM is carried, not copied.** It stays on the `ideas` row; every angle is written to it, no brief field restates or varies it, and a legacy subject carrying none has that absence reported rather than filled in.
...
- **The MECHANISM lives on the idea and is CARRIED, never authored or restated here (hard rule).** One mechanism per subject (`idea.mechanism`, required at Ideate approval, per `craft/doctrine` §2), inherited by every angle beneath it. Every angle is written **to** it — consistent with it, leaving the writer able to hit the mechanism beat — and no brief field restates, paraphrases, sharpens, softens, replaces or contradicts it. This skill never writes `mechanism` (not onto the brief, not onto the idea) and never varies it per persona: one subject sprouting contradictory mechanisms across its personas is what the idea-level home exists to prevent. An angle that cannot be written to it is dropped, not re-mechanised. A **legacy** subject approved before the doctrine may carry none — proceed, drop the mechanism-fidelity criterion, and **name the absence** in Step 7; never invent one.
...
- **A failed KB read STOPS the run (hard rule).** `get_knowledge` reports absent paths in `missing`; check it on every load (Steps 1c/1d). Retry a missing path once, then **STOP and name the doc**. There is no fallback: this skill holds no copy of any rule it applies, so proceeding would mean running on a remembered version — two sources of truth for a doctrinal rule is the drift this repo has already been burned by, and a stopped run is recoverable where a silently-stale one is not. `brand/personas`, `craft/doctrine`, `craft/awareness-framework`, `ad/layer-tones`, `craft/close-job` and `craft/coverage` each stop the run. The **single exception** is a persona **detail** doc: that persona is excluded and named as a KB gap, nothing is remembered in her place, and the rest of the roster is still evaluated.
...
- **Quality gate is hard.** Every persisted angle is rated ≥4 on distinctiveness / persona-fit-and-grounding / strategic sharpness / awareness fit / decision fidelity / authenticity / close-job compliance / mechanism fidelity / lead-openness, with a one-line Vietnamese `comment`. The last three carry **hard caps at 2**, alongside `Tránh`. Any ≤3 is dropped + regenerated (bounded at 2 attempts) or the batch is honestly reduced.
