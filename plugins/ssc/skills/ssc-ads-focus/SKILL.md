...
- If `plan` is non-null and `plan.retrospective` is a non-empty string — this is the **Measure → Focus carry-forward**. Read it as prose: which ad angles were winning (carry forward), which were fatigued/inefficient (drop or refresh), and what to try next. Let it shape the tactical bets — paid creative fatigues fast, so the retrospective's winning/fatigued split is the single strongest signal for this month's angle bets. **The retrospective grades each angle by its TIER on the tier's own KPI — read those grades tier-correctly (do not collapse them onto one metric):**
  ...
  - **L2 (awareness / omnipresence)** is graded on **CPM + delivery volume + delivery continuity** (`days_counted` against the cycle's *covered* days), never cost-per-purchase. A winning L2 angle has a CPM inside the KB's green band and delivery sustained across the covered days (and ~0 purchases is correct) — never drop an omnipresence angle for "low purchases." An L2 angle is "fatigued" on **rising CPM with flat-or-falling CTR, or delivery collapsing into a handful of the cycle's covered days**.
  - **There is NO frequency in the retrospective, and you must not look for one, ask for one, or invent one.** The ad surface emits only `reach_day_sum` (the sum of each day's unique reach — the same person counted once per delivering day), so no frequency can be computed from it; Measure states the gap outright ("tần suất không khả dụng — dữ liệu quảng cáo chỉ có tổng tiếp cận cộng dồn theo ngày (`reach_day_sum`), KHÔNG khử trùng lặp theo người"). Treat that line as the expected content, not a missing value. Concretely: **never apply a frequency threshold (no ">3.5", no band of any kind), never divide `impressions ÷ reach_day_sum`, never read `reach_day_sum` as people reached, and never write a frequency number into the tactics doc or the summary.** If a carry/drop call would have turned on frequency, make it on the CPM/CTR/continuity evidence the retrospective actually carries, and say plainly that the frequency signal was unavailable — an explicit gap is the correct output, a plausible-looking number is the defect this rule exists to prevent.
  - The retrospective also carries a **coverage** statement (uncovered days / provisional conversions). A short delivery span on an *uncovered* stretch is a measurement gap, not fatigue — do not drop an angle for it.
...
- Drop/refresh: [fatigued angle + the tier signal the retrospective actually named, e.g. "X (L2 — CPM tăng, CTR giảm)" or "Y (L2 — phân phối chỉ còn vài ngày trong kỳ đã đo)" or "Z (L3 — chi phí/đơn tăng)", or "none". Never a frequency — none is available.]
...
- **Tier-correct carry-forward.** When the retrospective carries tier grades, honour each grade's tier: cost-per-purchase judges only L1/L3 (protect/scale L3, keep L1 at funnel-intake); L2 omnipresence is judged on CPM + delivery volume/continuity and is never dropped for low purchases.
- **No frequency, ever (hard rule).** The retrospective carries no frequency and no `reach` — the ad surface emits only `reach_day_sum`, a non-deduplicated day-sum. Never apply a frequency threshold, never derive a frequency from `reach_day_sum`, never cite `reach_day_sum` as people reached, and never write a frequency figure into `tactics` or the summary. Where the retrospective says "tần suất không khả dụng", carry that gap forward as stated instead of filling it.
