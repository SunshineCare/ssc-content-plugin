...
metadata:
  ...
  section: ads
  capability: edit
  tools: [get_knowledge, search_knowledge, get_channel_plan, list_taxonomies, save_idea, delete]
---
...
- `comment` — a **one-line rationale for the `score`, written in natural Vietnamese** (shown next to the stars for a Vietnamese operator): the single biggest reason the concept is strong or weak — e.g. "Frame confession khớp L1 <persona của ad set>, góc against sắc" or "Trùng value+frame với concept khác trong slot, thiếu khác biệt". Always Vietnamese (never English); keep it short and honest; it should justify the number you gave.
...
**If any check fails:** Fix the violations by replacing the affected concept — `delete(entity='idea', id, expected_version)` the flawed draft, then save the corrected concept via a fresh `save_idea` call (`save_idea` is insert-only; re-calling it does not update the existing row). Do not finalise Step 6 until all 18 checks pass.
...
- **Propose-only.** `save_idea` creates DRAFT concepts only. NEVER calls `approve` (any entity, incl. `idea` and `channel_plan`), `publish_*`, or any scheduling tool; never uses `edit` to demote/unapprove; and NEVER flips a gate.
...
- **No auto-approval.** The human operator curates and approves concepts in the dashboard (the Ideas gate is per-concept `approve(entity='idea', …)` → `status='approved'`). After the Ideas gate, the agent proceeds to the ungated **Measure** step — there is no Schedule step in the ad flow.
