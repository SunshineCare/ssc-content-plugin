...
**Map the angle type to the audience first.** Before writing, diagnose the ad set's awareness stage from its tier/audience, then read off **`ad/awareness-framework`** (loaded in Step 3) which angle types work at that stage, and pick the hook accordingly. Do not put a warm offer hook on a cold audience, or a cold pain hook on a warm one. **Take the stage→angle mapping from the live doc — do not work from a remembered table**; it is the same framework `ssc-ads-brief` used to choose this angle, so reciting a stale copy is how the copy drifts off the brief it is supposed to realize.
