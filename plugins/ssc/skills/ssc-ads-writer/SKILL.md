...
### Step 1b: Resolve the brief's persona / route / awareness_stage — the angle's tuning inputs
...
(No `kind` filter — one call returns every kind.) Build `id → { code, label }` maps from the rows where `kind === 'persona'` and `kind === 'route'`. Then:
...
- **Target layer** (`brief.target_layer_term_id`) is the angle's **declared media home** — informational only. It is what a human later realizes as an ad-set placement on the dashboard side; **this skill never reads it to steer register, length, or density** (that is the one thing the old `build_spec`-driven tier branches did that this design retires — see Governance). Hold its label for the Step 9 summary if you want to show it; never branch drafting logic on it.

If `brief.persona_term_id` or `brief.route_term_id` is not in the map `list_taxonomies` returned (a stale/deleted taxonomy term), or any of `persona_term_id`/`route_term_id`/`awareness_stage` is `null` — a legacy brief predating this fan-out model — do not invent an id or a stage. Fall back to reading the persona/route/stage off the brief's narrative prose (`hook_direction`/`why_now`) as a documented inference, flag it plainly in the Step 9 summary, and proceed; never STOP for this alone.

**Every section this run tunes to THIS angle's persona + route + awareness_stage** — never to a fetched ad-set object, never re-diagnosed from scratch, and never parsed out of `why_now` (Step 5 covers this explicitly).
...
If the active section is **`copy`**, there is no earlier *copy-section* input — ground it in the **chosen angle brief** (its `hook_direction`/`core_message`/`why_now`/`story_moment`/`cta` + `angle_label`, held from Step 1c) plus its resolved persona/route (Step 1b) + KB + the Hook Formula Bank (the copy realizes the brief's angle and owns the hook). Use **only** that one brief — never pool across the idea's other briefs.
