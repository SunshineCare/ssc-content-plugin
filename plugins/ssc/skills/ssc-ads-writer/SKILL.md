...
metadata:
  ...
  section: ads
  capability: edit
  tools: [get_knowledge, get_brief, get_channel_plan, list_content, save_content]
---
...
The paired **`idea`** is the ad concept, returned FLAT (the same shape `get_idea` returns): its lifecycle core (incl. `id`, `status`, `channel`, `plan_id`), its ad detail as **top-level fields** (`ad_slot_id` — the `ad_plan_slots` row this concept fills — and `ad_notes`; there is **no** nested `detail` object and **no** `period` field), and its `tags[]` (each `{ term_id, kind, code, label }`).
...
- `idea.id` — held for the Step 9 summary and to derive the plan period in Step 1b. Content is **brief-keyed**, so it is **not** a save argument: `save_content` takes `brief_id`, never `idea_id`, and Step 2 reads `list_content(brief=<brief_id>)`.
...
Fetch the concept's ad set so the copy is tuned to its placement, objective, and audience. The idea has **no `period` field**, so resolve the plan period `YYYY-MM` in this order — **the brief's period wins**:

1. **The plan period stated in the brief's `why_now`** (Step 1c). `ssc-ads-brief` resolved a period when it wrote this angle and records it there. Use it.
2. **Else the month of the idea's `created_at`.**
3. **Else ask the operator** for the plan month (one question).

**Why the brief's period outranks `created_at`:** `ssc-ads-brief` prefers its own `date` input when it was invoked with one, so an idea *created* in one month for a plan in another (a June-created concept on the July plan) makes the two skills resolve **different periods → different `ad_slots` → a different `build_spec`**. You would then tune the copy to an ad set the angle was never written for, silently. Taking the brief's period keeps both skills on the same ad set by construction. If the two disagree, follow the brief and note it in the Step 9 summary.
...
The chosen angle brief — its **`id`**, **`angle_label`**, and five narrative fields (**`hook_direction`** / **`core_message`** / **`why_now`** / **`story_moment`** / **`cta`**) — is already in hand from `get_brief` (Step 1), gated approved. It is **the angle anchor** for every section this run: `copy` is grounded in **only** this brief, and `headline`/`description`/`image_content` lead from the approved copy while holding this same one brief as the angle. Use **only** this brief; never pool across the idea's other briefs.
...
**Then make a SECOND, equally mandatory call** for the two run-resolved docs. They are split out only because `get_knowledge` caps `paths` at 20 and the static list above already holds 19 — not because they are optional. **Do not skip this call: without it you are writing to the persona's name instead of her documented language, and with no awareness/sophistication filter at all.**
...
If the active section is **`copy`**, there is no earlier *copy-section* input — ground it in the **chosen angle brief** (its `hook_direction`/`core_message`/`why_now`/`story_moment`/`cta` + `angle_label`, held from Step 1c) plus the concept brief + `build_spec` + KB + the Hook Formula Bank (the copy realizes the brief's angle and owns the hook). Use **only** that one brief — never pool across the idea's other briefs.
...
**Map the angle type to the audience — READ the decision, do not re-make it.** The awareness stage and the angle type were already diagnosed by `ssc-ads-brief` and are recorded in **this brief's `why_now`** (Step 1c), which states the stage and the framework's rationale for why that angle type lands on it. **That decision is the approved one — take it from `why_now` and realize it.** Re-diagnosing here is not a safety net: it is how the copy silently ends up on a different angle type than the brief the operator approved, which is the one thing this skill must never do.

Use **`ad/awareness-framework`** (loaded in Step 3) to *realize* that stage — which hook patterns serve it, what the sophistication stance allows — not to re-decide it. **Take the mapping from the live doc, never a remembered table.**

- **If `why_now` names a stage/angle type** — use it. If your own reading of the ad set disagrees, the **brief wins**; note the discrepancy in the Step 9 summary so the operator can re-brief if the angle really is mis-staged. Never quietly override it.
- **If `why_now` does not name one** (a brief written before the brief-side diagnosis existed) — only then diagnose it yourself from the ad set's tier/audience against the live framework, and say so in the summary. Do not put a warm offer hook on a cold audience, or a cold pain hook on a warm one.
...
> **ADS ARE OPTIMIZED FOR CONVERSION — every section, every layer, no exceptions.** This is the channel's objective: `copy`, `headline`, `description`, and `image_content` all exist to move her toward the Messenger conversation and the order (`ad/platform-constraints`: CTA destination is a human consultant via Messenger, always; L1/L3 are graded on cost-per-purchase). **Ads convert; posts earn conversation** — the engagement register belongs to `ssc-post-produce` / `ssc-post-authority`, never here. No section may be soft, decorative, mood-only, or merely "on brand".
>
...
>
> **Converting is not CTA-stuffing.** `ad/headline-formulas` reserves CTA language for the *CTA mời gọi* formula on warm/L3 audiences — elsewhere the CTA button carries the action, and a short section spends its characters on the hook plus a concrete proof point instead. **The layer changes HOW a section converts (register), never WHETHER it does** — an L2 omnipresence set converts through person-led / lived-proof framing rather than offer framing.
...
- if active = **`image_content`** — `n_image_contents` versions (default **5**). Each is one **on-image COPY set** — text that sits **ON a visual, read at a glance in under two seconds**, so **every element is SHORT, punchy, and minimal-word**. These are **hard caps, gated in Step 7 — not targets to aim at.** A designer must fit this on an image, and Meta's 20%-area guidance (`ad/platform-constraints`) means every extra word costs reach.
  ...
  **The layer steers DENSITY and REGISTER — never hook strength** (from the `build_spec`, Step 1b), consistent with the awareness stage from Step 5: an **L2 omnipresence** ad set is reach/person-led, so weight its set toward **Minimal** and let its hook convert through a **person-led / lived-proof** framing rather than an offer framing; **L1 / L3** conversion sets can carry more **Standard** density and lean harder on proof. Weight the mix — never collapse it to one profile. **On every layer the headline is still a converting hook** (above): L2 changes *how* it converts, never *whether* it does.
...
- [ ] **Presses real advantages (sized to format)** — a `copy` weaves in **≥3 distinct** Cambridge proof points from `brand/proof-points` (one landing the concept's `against` match-up via `brand/positioning`); a `headline`/`description` carries the 1–2 that fit cleanly (its set covers ≥3 — checked in the loop below); an **`image_content` version carries as many as fit inside the word caps — typically its 3 bullets, and never at the cost of a cap**, with the ≥3-distinct requirement satisfied **across the set** rather than per version. A `copy` with <3 distinct proof points, or any variation leaning on nothing distinctive, cannot score ≥4. **Never pad an `image_content` version with a proof point to reach a count** — on-image, an unread third proof is worth less than a headline that lands.
...
- **Saved, not presented.** For the single active section, per-section DRAFT `content` rows via `save_content(channel='ad', brief_id, section, body, score, comment)` — the section's count (default 5 headlines / 5 copies / 5 descriptions / 5 image-content sets), every saved variation rated ≥4 with a Vietnamese comment. Saved immediately after scoring; there is no in-chat candidate presentation or revise loop. Saving persists drafts; it is NOT approval/selection.
...
- **The brief's decisions are READ, never re-made (hard rule).** The awareness stage, the angle type, and the plan period all come from the chosen brief's `why_now` — `ssc-ads-brief` diagnosed them when the operator approved that angle. Re-diagnosing here would let the copy land on a different angle type, or tune to a different ad set, than the approved brief specifies. Re-derive **only** when `why_now` does not state a value (a brief predating that contract), and disclose it in the summary. Where your own reading disagrees with the brief, the **brief wins** and the discrepancy is reported, never silently applied.
- **Anchored to one approved brief (hard rule).** Every section is written from the single approved angle brief named by `brief_id` — resolved via `get_brief(brief_id)`, which returns that brief and its owning idea in one call; STOP if the brief is missing or still a draft. `copy` is grounded in **only** that brief (its five narrative fields + `angle_label`), never pooled across the idea's other briefs; `headline`/`description`/`image_content` lead from the approved copy and hold the same one brief as the angle anchor.
...
- **Differentiation & proof (sized to format).** Each `copy` weaves in ≥3 distinct proof points from `brand/proof-points`; each `headline`/`description` carries 1–2 and the section's set surfaces ≥3 distinct (Step 7 set-coverage check); each `image_content` version carries as many as fit **within its hard word caps**, with ≥3 distinct met **across the set** — never crammed into one version. Every variation lands the concept's `against` match-up when present (`brand/positioning`) — concrete, not slogan. A `copy` or short-section-set under the minimum, or a variation leaning on nothing distinctive, cannot score ≥4.
- **`image_content` payload is sized to the visual, not fixed.** Only `HEADLINE:` is always emitted; `SUBHEADLINE:`/`BULLETS:` (0–3) follow the version's **density profile** — Minimal / Standard / Text-dominant — defaulting by layer (L2 omnipresence → Minimal; L1/L3 → Standard). The profile must **vary across the set** so the Text step has a real choice to make against the finished visual. Safe by construction — the `/ad` page's parser is lenient and its renderer conditional, so a HEADLINE-only body renders correctly; never pad a profile to fill the template.
...
- Requires the `edit` capability (plus `view` for the `get_brief` / `get_channel_plan` / `list_content` / `get_knowledge` reads).
