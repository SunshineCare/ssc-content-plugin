...
description: >-
  Authors the Composition step of the Cambridge Diet Vietnam ImageStudio image-prompt
  pipeline — the anchor-gated compose that brings the locked real identity and the
  real Cambridge packshot into one image, onto a selected Scene or around the anchors.
  Anchored to one approved briefId; saves via
  save_creative_prompt(layer:'composition'). Zero-credit — the operator clicks
  Generate in the ImageStudio.
metadata:
  type: skill
  stage: produce
