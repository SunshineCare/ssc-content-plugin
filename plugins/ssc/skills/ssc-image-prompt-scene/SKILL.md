...
description: >-
  Authors the Scene step of the ImageStudio image-prompt pipeline for Cambridge Diet
  Vietnam ads and posts. Anchored to one approved briefId, it proposes five scene
  setups, waits for the operator's pick, then writes that setup's text-to-image prompt
  and saves it via save_creative_prompt(layer:'scene'). Zero-credit — it never
  generates; the operator clicks Generate in the ImageStudio.
metadata:
  type: skill
  stage: produce
...
**Grounded on contents:** <"dựng CHÍNH trên <n> copy đã duyệt, có <các section khác> + brief hỗ trợ (chỉ lấy ý + tông, không nêu chữ)" | "chưa có nội dung đã duyệt — dựng theo brief + persona (nên chạy /ssc-ad <brief_id> copy — hoặc /ssc-post <brief_id> với bài viết — để sắc hơn)">
