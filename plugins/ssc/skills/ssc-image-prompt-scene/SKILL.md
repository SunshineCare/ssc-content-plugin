...
> **Two different authorities — do not conflate them.** The brief is the **ANGLE** authority: which angle this visual belongs to, always, non-negotiably (it is the `brief_id` lineage every creative is keyed on). The **approved contents are the MOMENT authority** whenever any exist: which concrete moment is depicted and in what emotional register. The brief's `story_moment` / `core_message` are the **fallback** for the moment, used when nothing is approved yet — not the preferred source. Approved copy is what the operator actually signed off and what the viewer will read beside this image, and it may have been edited in the dashboard away from the brief's original direction; a scene built on the brief while approved copy says something else is a visual that contradicts its own ad.

1. **The chosen angle brief** (Step 1) — `angle_label` + `hook_direction` / `core_message` / `why_now` / `story_moment` / `cta`. This is *the* angle; the scene expresses **this and nothing else**. It fixes WHICH ANGLE; where approved contents exist they fix which MOMENT within it.
2. **ALL APPROVED CONTENTS of the brief — the MOMENT + TONE authority (D4).** Ground the scene in **every** approved content section of this angle, in preference to the brief's own `story_moment`:
...
**Grounded on contents:** <"đã dùng nội dung đã duyệt (<các section có của channel này> — chỉ lấy ý + tông, không nêu chữ)" | "chưa có nội dung đã duyệt — dựng theo brief + persona (nên chạy /ssc.ads-produce <brief_id> copy — hoặc /ssc.post-writer <brief_id> với bài viết — để sắc hơn)">
