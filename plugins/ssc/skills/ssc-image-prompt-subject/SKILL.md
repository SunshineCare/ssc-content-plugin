...
description: >-
  Authors the optional Subject step of the Cambridge Diet Vietnam ImageStudio
  image-prompt pipeline — the persona's woman generated alone on a neutral ground, her
  face and pose locked as the anchor the Composition step later builds around.
  Anchored to one approved briefId; saves via save_creative_prompt(layer:'subject').
  Zero-credit — the operator clicks Generate in the ImageStudio.
metadata:
  type: skill
  stage: produce
