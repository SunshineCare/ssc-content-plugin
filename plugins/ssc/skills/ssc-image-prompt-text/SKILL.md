...
- **Weak hook** — a HEADLINE that is merely the anchor copy's opening sentence shortened, with no named formula behind it, is rejected however well it fits the cap. **Name the formula used in the `comment`.** A HEADLINE that fails `craft/headline-formulas`' competitor test, or that reads as a `Brand: feature` tagline, is rejected too.
...
- **An Approaches constraint marked `RÀNG BUỘC` on a `post`** — the binding rails held from `plan.context` in Step P3(b). This is the **highest-precedence** family, outranking the KB. Work through them one by one against each candidate, as that document words them; never skip one because a candidate is strong elsewhere. Its **boundaries** section rejects in **both directions**: a candidate that omits, truncates or paraphrases an element the boundaries require on every post is rejected exactly as one that does something they forbid. **Name the specific rail in the `comment`, in the Approaches' own words.**
...
Write the `comment` in **Vietnamese**, one line, naming the single biggest reason the candidate is strong or weak and the rule / doc it traces to — plus the **formula** the HEADLINE was written to, the **opening frame** it used, and the **proof row** backing the mechanism where one applies (with `(ngoài nhóm bằng chứng của cơ chế)` appended where that row sits outside the mechanism's own family). Use the full 1–5 range honestly; never nudge a number to make a set look ready.
...
Call: save_content
  ...
  coverage: { verdict: <pass|fail|pending>,
              ...
              axes_missing: ["<kind>", …],
              notes: "<one-line Vietnamese rationale>" }
  score:    <the integer 1–5 brand-fit signal — SECONDARY, never a ship gate>
  comment:  <the one-line Vietnamese rationale, carrying the formula, the opening frame and the proof row>
```
...
- **`body` / `score` / `comment`** — the Vietnamese block, the honest brand-fit rating, and the Vietnamese rationale. All persisted prose is **Vietnamese**.
