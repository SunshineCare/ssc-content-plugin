...
Call: save_content
  ...
  comment:  <Vietnamese rationale — at most 15 words, counted, plus the trailing
...
- **`body` / `score` / `comment`** — the Vietnamese block, the honest brand-fit rating, and the Vietnamese rationale under the Step P5(b) cap: **at most 15 Vietnamese words, counted**, carrying the reason alone, plus the one `· proof: …` tag where the mechanism beat is backed (outside the count; absent, never `NONE`, where there is none). The formula and the opening frame are recorded — in `terms[]` and in the report — never narrated here. Nothing enforces the cap server-side, and it never softens a judgement: what will not fit goes to the report, which has no cap. All persisted prose is **Vietnamese**.
