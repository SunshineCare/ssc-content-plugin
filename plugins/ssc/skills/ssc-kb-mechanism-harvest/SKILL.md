...
description: >-
  Harvests the mechanisms a period's BRIEFS actually settled and returns the genuinely new ones to the mechanism bank — the BrandOS `mechanisms` TABLE, read with `list_mechanisms` / `get_mechanism` and written with `save_mechanism` — so a mechanism authored at an angle because nothing in the bank fitted stops being re-invented in the next period that meets the same objection. The mechanism lives on `briefs.mechanism` and nowhere else: `list_briefs` and `get_brief` return it per angle, and this skill reads it there and only there. Membership is a SEMANTIC MATCH, never a join: there is no `briefs.mechanism_slug`, so a brief holds the Vietnamese sentence and nothing that says where it came from — this skill reads the bank live and compares MEANING, not wording, and every match it asserts is a judgement it must show its working for. A genuinely new mechanism becomes a `save_mechanism` call, which ALWAYS mints a `draft` and accepts no `status`: a draft is not supply, so nothing that reads the bank sees it until a human approves it in the dashboard, and that is what makes writing into the bank safe at all. A near-duplicate — the same causal claim in different words — SHARPENS the entry it matched in place with `edit(entity='mechanism', id, patch, expected_version)`, never a second entry saying the same thing and never a silent merge, under FOUR bounds that hold together: content fields only (`mechanism`, `fits`, `proof_family`, `notes`); NEVER `status` and NEVER `slug`; sharpening only, never repurposing an entry to a different meaning; and every edit reported with its BEFORE and AFTER, naming which entry was matched and why. That in-place edit is this plugin's one live-supply write that does not go through a proposal — it is deliberate, it is bounded, and it flips no gate. The run also reports the period's MECHANISM MIX measured over that period's briefs — per-mechanism concentration above roughly a quarter of the period's assets, and negative-valence share above a third — naming every breach; the audit is REPORT-ONLY, it re-mechanises nothing, re-opens nothing and edits no brief, and the correction is the operator's on briefs not yet approved. It holds no `approve` and no `unapprove` in any form, no `propose_knowledge_revision` (bank entries are table rows, not document sections), no `save_knowledge`, and no publish or schedule tool, so it cannot promote its own draft. It writes no usage count, no last-used period and no retired flag, and it retires NOTHING: a weak entry is a reported finding, never a removal. It tags each drafted entry with a `valence` read off the sentence's own framing against the vocabulary `craft/mechanism-bank` §2 defines (read live, never remembered), a `fits` taken from the attributed voice-of-customer item the mechanism was grounded in — described, never persona-named — and a `proof_family` taken from the proof route it was traced to and confirmed against `brand/proof-points`; an entry whose grounding no recorded source carries is REPORTED as a gap, never drafted. A failed bank or KB read STOPS the run and names what could not be read; it never diffs against a remembered bank and never reports an empty harvest as a clean one. Persisted prose is Vietnamese; field labels are structural English.
metadata:
  type: skill
  stage: harvest
