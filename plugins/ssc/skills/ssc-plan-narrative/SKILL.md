...
description: "Runs the NARRATIVE step of the Cambridge Diet Vietnam monthly plan head — authored last, and the artifact carrying the month's only approval. Synthesises the Review, Tactics and Research into a short Vietnamese document (300–500 words) that makes the month approvable in one read, written to month_plans.narrative. Propose-only — a human approves it, and that releases every linked channel."
metadata:
  type: skill
  stage: monthly-plan
