...
description: "Runs the RESEARCH step of the Cambridge Diet Vietnam monthly plan head — the month's one outward signal pass, consumed by every channel. Scans for what is true right now (calendar openings, competitor and platform moves, audience triggers, emergent topics) against the month's themes, and carries the quarter's market-sophistication read verbatim. Writes month_plans.research; ungated, propose-only."
metadata:
  type: skill
  stage: monthly-plan
