...
description: "Runs the REVIEW step of the Cambridge Diet Vietnam monthly plan head — the first of its four steps and the system's only look-back. Ranks taxonomy terms, never metrics, from three lenses that are never blended: organic posts, boosted posts, and ads by layer. Writes a Vietnamese markdown report to month_plans.performance_review, proposing scale/maintain/drop per term. Propose-only; sets no gate."
metadata:
  type: skill
  stage: monthly-plan
