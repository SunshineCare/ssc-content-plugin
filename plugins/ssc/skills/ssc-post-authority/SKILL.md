...
- `score` — **an integer 1–5 recording BRAND FIT only, and it decides nothing.** It is a curation signal:
  ...
  proof backing — stated below in the `copy` list and again in the `image_content` list. Naming that single
...
- `comment` — **a one-line Vietnamese rationale** (the persisted prose a Vietnamese operator reads in the
  ...
  **omitted** in two cases: where `brief.mechanism` is blank (whose absence is named instead), and where the
  version's density profile emits no bullets to carry a proof row (inert, not a miss, and not named as one).
  ...
  absence — survives first, then the `rules/person-rule` §4 frame name (and, on `image_content`, the
  `craft/headline-formulas` formula that section's hook item asks for), and the prose rationale is
...
- **Is the beat proof-backed? (`copy`) — this one CAPS at ≤3; it does not reject.** It is the only criterion
  ...
  `brand/proof-points`. It binds `copy`; for `image_content` the same requirement is stated in that list's
  proof item, where it is additionally **inert** on a density profile that emits no bullets. The mechanism
  ...
  read which of the families in `brand/proof-points` § Bốn Nhóm Bằng Chứng the mechanism sentence argues from,
...
**Judging an `image_content` candidate** — the same two gates and the same one signal, adjusted for a format that carries no prose. Every item below is a **rejection**, never a rating adjustment — with the one exception carved out inside the proof item, the mechanism beat's proof backing, which declares its own force in its first clause and **CAPS at ≤3 rather than rejecting**. Every other item rejects; inside the proof item the backing requirement caps, the competitor-swap clause cuts, and the item's remaining clauses are sizing guidance the word caps and the floor already enforce — they raise no rejection of their own:
...
- **Proof (sized to format, and it YIELDS to brevity):** carry as many distinct Cambridge proof points as fit **inside the word caps** — typically the bullets, where the bullets ARE the proof list (the one place a proof list is the intended format, not a bare-list failure). The **≥3-proof requirement is a question of the whole SET** (`craft/coverage` §4.2) — never crammed into one version, and there is **no per-version proof requirement**. **Never pad a version with a proof point to reach a count, and never breach a word cap to fit one** — on-image, an unread third proof is worth less than a headline that lands. A bullet that survives the competitor test — swap Cambridge for another wellness brand and it still reads true — is **cut rather than scored down, and replaced by the concrete form the live `brand/proof-points` row actually carries**, read this run; where the assigned family has no usable concrete row this run, say so rather than keeping the generic paraphrase because it fills the slot. **The version keeps the density profile it was assigned** — the cut takes the bullet's swappable phrasing, never the version's profile, so a Standard version stays Standard and the set's profile span is untouched.

  - **Mechanism backing — this clause CAPS at ≤3 and never rejects, exactly as it does for `copy`.** Where the bullets carry the mechanism's proof, the bullet standing in for the mechanism beat leans on at least one row of the live `brand/proof-points` (`§ Bảng Proof Points`), read this run, and the `comment` names which row. **Start the search in the mechanism's own proof family** — read which of the families in `brand/proof-points` § Bốn Nhóm Bằng Chứng the mechanism sentence argues from, judged live against that section, and look there first; the family is **read from the sentence, never resolved by a `mechanisms` bank lookup**, since provenance is report-only and the brief carries the sentence alone. **It is a starting point, not a fence** — a row from beyond the family is legitimate backing, **not** a miss and **never** a reason to cap; the family rule adds only the **report** obligation to say when the backing row sits outside the mechanism's own family. **Backing the mechanism reassigns, widens or overrides no slot's planned `proof_device` family.** **Where the density profile emits no bullets** (a version that is HEADLINE-only, or HEADLINE + SUBHEADLINE) the requirement is **inert — not a miss**: the format has nowhere to put a proof row, and the caps above outrank proof density. It is inert on a **blank** `brief.mechanism` too, whose absence is named rather than invented.
