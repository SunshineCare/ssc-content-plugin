...
- `comment` — **the Vietnamese rationale** persisted on the row (the prose a Vietnamese operator reads in
  ...
  > **At most 500 characters, counted** — the reason this is strong or weak. How many sentences those
  ...
  (Steps 4 and 7) — it is judged exactly as before and simply is not narrated in the comment. `save_content`
  and `edit(entity=content)` both refuse a write whose `comment` exceeds the cap, so self-check the length
  before calling rather than relying on the refusal round-trip. For a **rejected** variation, the rationale you hand back names
...
- **Is the beat proof-backed? — this one CAPS at ≤3; it does not reject.** It is the only criterion
  ...
  variation's Vietnamese `comment` carries that row as its one compact trailing tag, outside the character cap
...
Then show the **SET-level coverage verdict**, once, above or below the list: `pass` / `fail` / `pending`, the axis kinds the set spans, the axis kinds it did **not** span, and the Vietnamese `notes` you will persist with it — the same capped rationale (Step 3b: at most 500 characters, counted). Anything the cap will not hold goes into the presentation's own prose, which has no cap. **State plainly that this verdict — not the ratings — is what decides whether the set ships**, and that a set failing it does not ship even though every member passed the floor.
...
Call: save_content
  ...
  comment:  <the Vietnamese rationale you wrote for this candidate — at most 500 characters, counted,
  ...
  coverage:                       # SET-level — pass it on the FIRST insert of the set ONLY
    ...
    notes:        <Vietnamese rationale for the verdict — at most 500 characters, counted>
```
...
- `comment` — **the Vietnamese rationale** (MUST be Vietnamese), under the Step 2 cap: **at most 500 characters, counted**, carrying the reason alone, plus the one `· proof: …` tag where the mechanism beat is backed (outside the count; absent, never `NONE`, where there is no mechanism beat). `save_content` refuses the write if this is over the cap — self-check before calling — and it never softens a judgement: what will not fit goes to the presentation and the Step 7 summary, which have no cap.
- `channel` — always `post`.
...
**Set coverage (the verdict that decided whether this set ships):** <pass | fail | pending> — axes judged: <the craft/coverage §5 subset for `copy`> · axes not spanned: <axes_missing, or "—"> · <the persisted Vietnamese `notes`, at most 500 characters> · <anything the cap could not hold, stated in full here>. Length band judged ordinally (`craft/coverage` §6). `opening_frame` is recorded, not an axis (§4.1).
...
- **Every persisted `comment` and coverage `notes` within its 500-character cap** — the reason alone, plus the one `· proof: …` tag where the mechanism beat is backed; no judgement softened to fit it
