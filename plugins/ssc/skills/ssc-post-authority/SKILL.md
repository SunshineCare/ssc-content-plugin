...
description: The AUTHORITY (brand/quality gate) of the standalone Cambridge Diet Vietnam post-writer production workflow, working ONE section per invocation — `copy` (the mandatory cold start) or `image_content` (the structured on-image copy the ImageStudio's Text layer renders, GATED on ≥1 approved copy), named via an optional section argument or auto-picked as the next open section. For `copy` it takes the N draft variations the writer (ssc-post-produce) just drafted in-conversation for ONE post idea; for `image_content` it drafts the N on-image versions ITSELF (there is no writer step for on-image copy), grounded in the post's live approved copies. It loads the GOVERNING FRAME first (the post channel plan's Approaches `context` for the post's period via get_channel_plan, then the month plan's research + tactics via get_month_plan) and judges EACH candidate PASS/FAIL against that frame FIRST — every constraint the Approaches marks binding is a REJECTION when tripped, in both directions (an element its boundaries require on every post is a rejection when missing, not only when violated) — then against the whole rules/voice/content KB slices (incl. voice/founder-voice — every variation must be written AS Kiều My) + programme/kieu-my-story (founder-story authenticity) + brand/angles + channels/facebook + the tagged persona's brand/persona-<slug> detail doc + the cross-channel craft doctrine read live and never restated (craft/doctrine §6 routing + §2 mechanism, craft/copy-floor's six-item pass/fail per-asset floor, craft/coverage §4–§7's set-level coverage, craft/awareness-framework §5's craft bar, craft/close-job §2's job vocabulary assigned by craft/cta §6's funnel-stage close rule, craft/cta §2's two-halved urgency law) + rules/person-rule — the GRAMMATICAL opening rule that rules/banned-words structurally cannot catch: its §2 test and §4 permitted frames are checked on EVERY variation in both sections, and the frame used is RECORDED on every persisted row (recorded, never spanned — craft/coverage §4.1 rules opening_frame out as a coverage axis). A failed KB read STOPS the run and names the document that could not be read; it never proceeds from prose, memory or a cached copy. A floor failure is a REJECTION, not a low rating — a rejected variation is never saved, approved or published, and a good rating never saves one; each rejection is regenerated ON THE SAME AXIS POSITION it held and the whole set is re-judged. The SET is then judged on coverage over the post-legal axes craft/coverage §5 assigns the section, with length band judged ORDINALLY (§6), and a set that fails coverage does not ship even when every member passes the floor. Brand fit survives only as a 1–5 CURATION signal that orders/recommends variations to the operator — never a gate, never why a set ships. It then PRESENTS the candidate set to the operator in chat (numbered body + floor verdict + opening frame + brand-fit signal + comment, plus the set's coverage verdict) and PAUSES for review — the operator either requests revisions (regenerate, re-judge the variation AND the set, re-present) or gives the go-ahead, and ONLY THEN persists the set via save_content (channel='post', the target section STAMPED on every row — 'copy' or 'image_content' — plus the post's brief_id, one insert per candidate carrying body + score + comment, the comment naming the opening frame used, plus the SET-level `coverage` verdict recorded once per (brief, section) set). Saving persists DRAFTS to curate — it is NOT a gate approval. Propose-only; never approves, publishes, or flips a gate.
metadata:
  type: skill
...
>
> **A good rating does not save a variation and a bad one does not sink it** — only the floor does. The old
> loop (drop everything rated ≤3, regenerate until N are ≥4) is **retired**: it decided survival by a
> rating and constrained the replacement only to "same angle", which — the angle being fixed across every
> variation of a set anyway — bound nothing, so the surviving sets collapsed toward sameness.
...
- **`pending`** is the honest verdict when an axis could not be judged because it was never recorded (a legacy row, or a section the run could not determine a value for). Report it as unjudged — never as spanned, never as collapsed — and say so in the presentation and the summary.
...
- **A rejected variation's replacement holds its AXIS POSITION (hard rule).** The replacement occupies the same value on each judged axis, and the **whole set is re-judged on coverage** afterwards. Matching the set's angle is **not** sufficient — the angle is fixed across every variation anyway, so it binds nothing; that is precisely why the retired loop's survivors collapsed toward sameness. The opening frame is **not** an axis to preserve: every replacement declares and is checked on its own (`rules/person-rule` §4).
- **Legacy rows proceed and are reported, never re-opened (hard rule).** Ideas, briefs and content approved **before** this change stay valid; production on them proceeds and they are never blocked or re-judged for a doctrinal input that did not exist at approval — a missing mechanism, an unrecorded axis, an undeclared funnel stage. The run's report **names each absent input** (Step 7) and **invents none of them**. New approvals are held to the new bar.
