...
- `brand/proof-points` — already in the list above; for `image_content` it is the **source of the 3 bullets** (real, compliant proof: 60 năm, chuẩn EU 2016/1413, 26 vi chất, chuyên viên 1:1 đồng hành, chương trình 6 bước, the app).
...
**The body shape is a fixed contract** — the workspace parses it, and the ImageStudio's Text layer renders it. Emit each version's `body` in **exactly** this structure:

```
HEADLINE: <hook rất ngắn>
SUBHEADLINE: <USP/proof chính, hoặc lời giải trả cho headline>
BULLETS:
- <proof point 1>
- <proof point 2>
- <proof point 3>
...
The `HEADLINE:` / `SUBHEADLINE:` / `BULLETS:` labels are **fixed structural markers** (ASCII, exactly as written); every value is **Vietnamese**.

**This is text that sits ON an image, read at a glance — so every element is SHORT.** The bar is the **word-count target on each element below**; keeping to it also keeps the block under the ~20% image-coverage guideline in `ad/platform-constraints`:

- **HEADLINE** — a very short, bold hook (aim ~3–6 words, one glance), distilled from the anchor copy's hook. No sub-clauses.
- **SUBHEADLINE** — one short phrase (aim ~4–8 words): the key USP/proof, or the solution that pays off the headline. A phrase, not a sentence.
- **BULLETS** — exactly **3** distinct proof points from `brand/proof-points`, each a terse keyword phrase (aim ~2–5 words) — e.g. "60 năm khoa học", "Chuẩn EU · 26 vi chất", "Chuyên viên 1:1 đồng hành". Never a sentence or a paragraph.

Cut every word that isn't load-bearing. Vary the hook/angle across the N versions while keeping the same post spine. Every rule that binds copy still binds here — `rules/banned-words`, `rules/compliance` (spell out "nghiên cứu lâm sàng độc lập", never the "RCT" acronym; **26** not 25; no fabricated number), `rules/food-placeholder`, and the authenticity guardrail (never put a fabricated story, result, or quote in Kiều My's or any real person's mouth — verify founder specifics against `programme/kieu-my-story`). The register shows in word choice, not in narration: these are terse proof phrases, not first-person prose, so judge voice as **tone fit** rather than as first-person founder narration.
...
- **Structure + brevity cap:** a body that is not in the exact `HEADLINE:` / `SUBHEADLINE:` / `BULLETS:` shape with **exactly 3** bullets — or that is wordy enough to fail the glance test (sentences instead of phrases, judged against the **Step 1b word-count targets**) — caps at **≤3**. The workspace parses this shape; a malformed body renders as raw text.
- **Proof cap:** fewer than **3 distinct** Cambridge proof points across the bullets + subheadline caps at ≤3 (here the bullets ARE the proof list — this is the one place a proof list is the intended format, not a bare-list failure).
...
- **`image_content` is TEXT, and you draft it yourself.** It renders no picture and carries no image url — it is the on-image copy in the fixed `HEADLINE:` / `SUBHEADLINE:` / `BULLETS:` shape (exactly 3 bullets), anchored to a LIVE APPROVED copy's hook and kept short enough to read at a glance. There is no writer hand-off for this section: you draft, score, present, and (on the go-ahead) save. Saving it is still **not** approving it, and you never hand it to the image engine — the operator approves it in the Image Content stage and generation is a human action in the studio.
...
- **All persisted prose in Vietnamese.** The saved `body` — the post copy, **and** every line of an `image_content` block (headline, subheadline, all 3 bullets; only the `HEADLINE:` / `SUBHEADLINE:` / `BULLETS:` markers are ASCII) — AND the saved `comment` (rationale) MUST be Vietnamese. Chat-side reasoning/analysis and the in-chat review dialogue may stay the operator's language; nothing written to the row may.
