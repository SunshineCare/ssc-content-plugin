...
description: >-
  Runs the IDEATE step of the Cambridge Diet Vietnam Posts channel in three
  rounds, one per invocation, each ending at an operator checkpoint: the
  month's pillar split, then one titled draft idea per planned post, then
  the post's brief — its hero, its one angle, and the mechanism
  ssc-brief-core settles for that angle. Gated on approaches_approved.
  Propose-only — it approves no idea and no brief.
metadata:
  type: skill
  stage: post-pipeline
