...
This is the **produce** step of the produce ⇄ authority production loop (**resolve → produce → authority scores → PRESENT in chat → operator review/revise → SAVE on go-ahead → STOP**). You feed clean candidate copy to the authority step, which scores each variation (1–5), writes the persisted Vietnamese rationale — **at most 500 characters, counted**, so hand it structured facts rather than prose to quote — drops + asks you to regenerate the ones that **fail the floor or a channel rejection** — a rating on its own never drops a variation — until N are strong, then **presents the set to the operator in chat and waits**. Nothing is persisted until the operator gives the go-ahead. **Do not do the authority's job and do not persist** — leave `score`, `comment`, and saving to the authority; produce (and revise) variations only.
...
- the **`opening_frame`** this variation used, named as `rules/person-rule` §4 names it — the
  authority checks it, shows it per candidate in its presentation and carries it in its run
  summary, which is how every post variation's permitted opening frame is recorded. It is
  **recorded, not spanned** (`craft/coverage` §4), and it is **not** narrated in the row's
  capped `comment`, so this hand-off line is how it reaches the record at all;
...
- the **proof row backing this variation's mechanism beat**, named as `brand/proof-points`
  ...
  **Never name a loosely related row to fill the line** — a fabricated backing hides the cap
  ...
  and a named row is what the authority persists as the row's one compact trailing tag
  (`· proof: <row as the live doc names it>`), outside its comment's 500-character cap; the three
  other states carry **no tag at all**;
...
**Hand these over as structured lines, not as prose for the authority to quote.** The rationale it persists is capped:

> **At most 500 characters, counted** — the reason this is strong or weak. How many sentences those words form is its call; there is no one-line rule. Nothing else goes in it: not the rule or doc it traces to, not the formula, not the opening frame, not the axis terms (those are carried by `terms[]`, the coverage record and this run's report). Where the mechanism beat leans on a proof row, one compact tag follows it, outside the count — `· proof: <row as the live doc names it>`, plus `(ngoài nhóm bằng chứng của cơ chế)` where the row sits outside the mechanism's own family; with no mechanism beat there is **no tag at all**. **The cap never changes a judgement:** a floor failure is still a REJECT, a score is still honest, and a fault that does not fit goes to the run report — never a merged vague phrase, never a softened verdict.

That is why the frame, the axis positions and the proof row travel on their **own** lines above: the persisted comment has no room for them, and this report — which has no cap — is where they belong. The authority's `save_content`/`edit(entity=content)` calls refuse a `comment` over the cap, so both skills self-check length before writing rather than relying on a refusal round-trip. **It never softens what comes back to you either:** a variation that fails the floor returns as a full REJECT with the failing item named at whatever length it takes, never as a shortened note or a down-rated pass.
...
---
<N> Vietnamese copy variations drafted (propose-only, UNSAVED, none scored/approved). Next: ssc-post-authority scores each (1–5) + writes the persisted Vietnamese comment (≤500 characters, counted), drops + asks me to regenerate any that fail the floor or a channel rejection (a rating on its own never drops one), then PRESENTS the set to the operator in chat and waits — the operator reviews and either asks me to REVISE named variations (I regenerate in-conversation, still unsaved) or gives the go-ahead, at which point the authority saves the set as drafts via save_content. A human then selects + approves one in the workspace.
```
...
- Per variation, the **`opening_frame`** it used (named as `rules/person-rule` §4 names it) plus its position on each coverage axis — handed to the authority so **every post variation it judges records the permitted opening frame it used**, in its presentation and its run summary rather than in the row's capped `comment`
...
- **The rationale you hand over is capped where it lands (hard rule).** The authority's persisted `comment` and its coverage `notes` each run to **at most 500 characters, counted**, carrying the reason alone — the rule or doc it traces to, the formula, the opening frame and the axis terms are carried by the coverage record, the presentation and the run report instead. Exactly one thing may follow a comment, outside the count: the `· proof: …` tag naming the row that backs the mechanism beat, with the out-of-family marker where it applies; the other three backing states carry **no tag at all**. So hand over the frame, the axis positions and the proof row as their **own** lines (Step 4), never as prose to be quoted. The cap is enforced server-side (`save_content` and `edit(entity=content)` both refuse a `comment`/`coverage.notes` over 500 characters), so hand the authority structured facts that already fit rather than relying on a refusal round-trip. **The cap never changes a judgement**: a floor failure still comes back to you as a REJECT with the failing item named in full, never as a shortened note or a quietly down-rated pass.
