...
save_idea(
  ...
  comment   = <rationale for the score, in Vietnamese, at most 15 words + the proof tag — see Field guidance>,
...
- `comment` — the rationale for the `score`, written in natural Vietnamese (never English). **At most 15 Vietnamese words, counted** — the reason this is strong or weak. How many sentences those words form is your call; there is no one-line rule. Nothing else goes in it: not the rule or doc it traces to, not the formula, not the opening frame, not the axis terms (those are carried by `terms[]`, the coverage record and this run's report). Where the idea's claim leans on a live proof family — or on real product paperwork — one compact tag follows it, outside the count: `· proof: <family or paperwork as the live doc names it>`; with no claim to trace there is **no tag at all**. **The cap never changes a judgement:** a floor failure is still a REJECT, a score is still honest, and a fault that does not fit goes to the run report — never a merged vague phrase, never a softened verdict. E.g. "Hook mở đầu mạnh, khớp giai đoạn Do dự của <persona>" or "Góc hơi chung, thiếu khoảnh khắc cụ thể".
...
- `detail.coreMessage` — the strategic direction (not a headline). One sentence stating what the viewer takes away — a belief, a reframe, or a transformation signal. **Whatever it claims must trace.** Name the live `brand/proof-points` proof family (or the real product paperwork) the claim rests on as the trailing proof tag on the idea's `comment`, and press it only as far as that family's own stated limit allows. An idea whose core message traces to neither is **untraceable and is refused** (check 12) — rewrite it onto something that traces, or drop the claim; never invent a figure, and never recall one from this file, which carries none.
